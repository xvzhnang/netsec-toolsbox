...
pub fn read_icon_file(icon_path: String) -> Result<String, String> {
    ...
    let icon_file_path = if icon_path.starts_with("icons/") {
        ...
        log::info!(
            "read_icon_file: 相对路径 (icons/)，完整路径: {}",
            full_path.display()
        );
        full_path
        ...
        log::info!(
            "read_icon_file: 相对路径 (.config/icons/)，完整路径: {}",
            full_path.display()
        );
        full_path
    } else {
    ...
    log::info!(
        "read_icon_file: 文件存在，开始读取: {}",
        icon_file_path.display()
    );

    // 读取文件
    let file_data = fs::read(&icon_file_path).map_err(|e| {
        let error_msg = format!(
            "读取图标文件失败: {} (路径: {})",
            e,
            icon_file_path.display()
