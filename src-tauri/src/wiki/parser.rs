...
impl MarkdownParser {
  ...
  pub fn render(&self, markdown: &str) -> Result<RenderResult, String> {
    ...
    // 先提取目录结构（从原始 Markdown）
    let toc = extract_toc_from_text(markdown);
    
    // 使用 Monokai 主题
    let theme = self.theme_set.themes.get("Monokai").unwrap_or_else(|| {
      self.theme_set.themes.values().next().unwrap()
    });
    
    // 解析 Markdown 并处理代码块高亮
    ...
    for event in parser {
      match event {
        ...
        pulldown_cmark::Event::End(pulldown_cmark::Tag::CodeBlock(_)) if in_code_block => {
          // 高亮代码块
          let highlighted = highlight_code(&code_block_buffer, &code_language, &self.syntax_set, theme);
          html_output.push_str("<pre><code");
          if !code_language.is_empty() {
            html_output.push_str(&format!(" class=\"language-{}\"", html_escape(&code_language)));
          }
        ...
        _ if !in_code_block => {
          // 对于非代码块内容，转换为 HTML
          let mut single_event_html = String::new();
          html::push_html(&mut single_event_html, std::iter::once(event));
          html_output.push_str(&single_event_html);
        }
        _ => {
          // 代码块内的其他事件（如换行）忽略，因为我们已经提取了文本
        }
      }
    }
...
fn extract_toc_from_text(text: &str) -> Vec<TocItem> {
  ...
  let mut stack: Vec<usize> = Vec::new(); // 存储索引而不是引用
  ...
  for line in text.lines() {
    ...
    if trimmed.starts_with('#') {
      ...
      if level >= 1 && level <= 6 {
        ...
        let text = if trimmed.len() > level_usize {
          trimmed[level_usize..].trim().to_string()
        } else {
          String::new()
        };
        ...
        while let Some(&parent_idx) = stack.last() {
          if toc[parent_idx].level < level {
            toc[parent_idx].children.push(item.clone());
            inserted = true;
            break;
          } else {
            stack.pop();
  ...
  // 只返回顶级项目
  toc.into_iter().filter(|item| item.level == 1).collect()
}
  ...
  for line in markdown.lines() {
    if line.trim().starts_with("```") {
      if in_code_block {
        ...
        // 开始代码块
        code_language = line.trim()[3..].trim().to_string();
        in_code_block = true;
      }
...
fn highlight_code(
  ...
  for line in LinesWithEndings::from(code) {
    ...
    for (style, text) in ranges {
      ...
      let r = (color.r as u32) & 0xFF;
      let g = (color.g as u32) & 0xFF;
      let b = (color.b as u32) & 0xFF;
