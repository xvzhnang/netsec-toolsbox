...
impl WorkerState {
    ...
    pub fn can_accept_request(&self) -> bool {
        matches!(self, WorkerState::Ready | WorkerState::Idle | WorkerState::Degraded)
    }
    ...
    pub fn is_ready(&self) -> bool {
        matches!(self, WorkerState::Ready | WorkerState::Idle | WorkerState::BusyStreaming | WorkerState::BusyBlocked | WorkerState::Degraded)
    }
}
...
impl GatewayWorker {
    ...
    /// 记录失败请求
    pub fn record_failure(&mut self, _is_timeout: bool) {
        let mut metrics = self.metrics.lock().unwrap();
...
impl GatewayPool {
    ...
    pub fn select_worker(&mut self, client_id: Option<&str>) -> Option<Arc<Mutex<GatewayWorker>>> {
        ...
        if let Some(cid) = client_id {
            ...
            // 特殊检查：如果 Worker-0 处于 Unhealthy 状态，跳过它
    ...
    fn start_worker(worker: &mut GatewayWorker) -> Result<String, String> {
        ...
        if let Some(stderr) = child.stderr.take() {
            ...
            thread::spawn(move || {
                ...
                for line in stderr_reader.lines() {
                    match line {
                        Ok(line) => {
                            // 关键优化：检测 [READY] 消息，从 READY 转为 IDLE（无 HTTP 健康检查的关键）
                            if line.contains("[READY]") {
                                let current_state = WorkerState::from(worker_state.load(Ordering::Relaxed));
                                if current_state == WorkerState::Ready || current_state == WorkerState::Init {
                                    worker_state.store(WorkerState::Idle as u8, Ordering::Relaxed);
                                    let mut metrics = worker_metrics.lock().unwrap();
                                    metrics.last_heartbeat = Some(Instant::now());
                                    log::info!("[Gateway Pool] [Worker-{}] 检测到 READY 消息，状态转为 IDLE", worker_id);
                                }
                            }
        ...
        // 等待进程启动后，通过进程状态检查转为 READY（无需 HTTP 调用）
        std::thread::sleep(Duration::from_millis(1500));
        
        // 检查进程是否仍在运行（无 HTTP 调用，非阻塞）
        if let Some(ref mut child) = worker.process {
            match child.try_wait() {
                Ok(None) => {
                    // 进程运行中，转为 READY 状态（等待 stderr 中的 [READY] 消息确认服务就绪）
                    worker.set_state(WorkerState::Ready);
                    log::debug!("[Gateway Pool] Worker-{} 已就绪（进程运行中，等待服务就绪消息）", worker.id);
                }
                Ok(Some(_)) => {
                    // 进程已退出，转为 Dead
                    worker.set_state(WorkerState::Dead);
                    log::warn!("[Gateway Pool] Worker-{} 进程在启动后退出", worker.id);
                }
                Err(_) => {
                    // 无法检查状态，保持 Init
                    log::warn!("[Gateway Pool] Worker-{} 无法检查进程状态", worker.id);
                }
            }
        } else {
            worker.set_state(WorkerState::Dead);
    ...
    pub fn diagnose_worker(&self, worker_id: usize) -> String {
        ...
        diagnostics.push(format!("  熔断器打开: {}", worker_guard.circuit_breaker_open));
        if let Some(opened_at) = worker_guard.circuit_breaker_opened_at {
            diagnostics.push(format!("  熔断器打开时间: {:?} 前", opened_at.elapsed()));
        }
        
        let metrics = worker_guard.metrics.lock().unwrap();
        diagnostics.push(format!("  连续失败次数: {}", metrics.consecutive_failures));
        diagnostics.push(format!("  最近失败率: {:.2}%", metrics.recent_fail_rate * 100.0));
    ...
    pub fn health_check_worker(&self, worker_id: usize) -> bool {
        ...
        if is_alive && heartbeat_ok {
            ...
            if consecutive_failures >= 5 {
                ...
                log::warn!("[Gateway Pool] Worker-{} 心跳超时，连续失败 {} 次，打开熔断器", worker_id, consecutive_failures);
            } else {
                worker_guard.set_state(WorkerState::Unhealthy);
            }
        }
        ...
        if consecutive_failures >= 5 {
            ...
            // Worker-0 特殊处理：输出完整诊断信息
            if worker_id == 0 {
                log::error!("[Gateway Pool] Worker-0 熔断器已打开，诊断信息:\n{}", self.diagnose_worker(0));
            }
        } else {
            worker_guard.set_state(WorkerState::Unhealthy);
        }
    ...
    pub fn start_health_check_thread(&self) {
        ...
        if HEALTH_CHECK_STARTED.compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst).is_err() {
        ...
        thread::spawn(move || {
            log::info!("[Gateway Pool] 健康检查线程已启动（无 HTTP 模式，仅进程状态和心跳检查）");

            loop {
                thread::sleep(Duration::from_secs(30)); // 每 30 秒检查一次（进一步减少频率，避免阻塞）
                ...
                for idx in 0..pool_size {
                    ...
                    if is_alive && heartbeat_ok {
                        ...
                        // 如果熔断器打开，尝试恢复
                        if worker_guard.circuit_breaker_open && worker_guard.should_attempt_recovery() {
                            log::info!("[Gateway Pool] Worker-{} 熔断器半开，尝试恢复", idx);
                            worker_guard.half_open_testing = true;
                            worker_guard.circuit_breaker_open = false;
                            worker_guard.circuit_breaker_opened_at = None;
                        ...
                        // 进程运行但心跳超时，标记为 Unhealthy（但不立即重启）
                        let mut metrics = worker_guard.metrics.lock().unwrap();
                        metrics.consecutive_failures += 1;
                        drop(metrics);
                        worker_guard.record_failure(false);
                        ...
                        if consecutive_failures >= 5 {
                            ...
                            worker_guard.circuit_breaker_opened_at = Some(Instant::now());
                            worker_guard.set_state(WorkerState::Unhealthy);
                            log::warn!("[Gateway Pool] Worker-{} 心跳超时，连续失败 {} 次，打开熔断器", idx, consecutive_failures);
                        }
                    }
                }
            }
    ...
    pub fn get_models_cached(&self) -> Result<Vec<String>, String> {
        // 1. 先检查缓存
        {
            let cache_guard = self.model_cache.lock().unwrap();
        ...
        let worker = self.workers.iter()
            .find_map(|w| {
                let wg = w.lock().unwrap();
                if wg.is_healthy() && wg.status().can_accept_request() {
                    // 跳过 Worker-0 如果它处于 Unhealthy 状态
                    if wg.id == 0 && wg.status() == WorkerState::Unhealthy {
                        return None;
                    }
                    Some(Arc::clone(w))
                } else {
        ...
        // 5. 更新缓存
        {
            let mut cache_guard = self.model_cache.lock().unwrap();
    ...
    pub fn forward_request(&mut self, method: &str, path: &str, body: Option<&[u8]>, headers: Option<&[(&str, &str)]>) -> Result<(StatusCode, Vec<u8>), String> {
        ...
        for attempt in 0..max_retries {
            ...
            let worker = match self.select_worker(None) {
                Some(w) => {
                    let wg = w.lock().unwrap();
                    // 特殊检查：如果请求是 /v1/models 且选中的是 Worker-0，且 Worker-0 处于 Unhealthy 状态，跳过它
                    if path == "/v1/models" && wg.id == 0 && wg.status() == WorkerState::Unhealthy {
                        ...
                        if attempt < max_retries - 1 {
                            ...
                            return Err("Worker-0 处于 Unhealthy 状态，且没有其他可用的 Worker".to_string());
                        }
                    } else {
                        drop(wg);
                        w
            ...
            // 恢复 Worker 状态（快速更新，避免阻塞）
            {
                ...
                let consecutive_failures_before = metrics.consecutive_failures;
                ...
                match &response_result {
                    ...
                    Err(_) => {
                        ...
                        let consecutive_failures = metrics.consecutive_failures;
                        drop(metrics);
                        wg.record_failure(elapsed >= timeout);
                        
                        // 如果连续失败过多，标记为 Degraded
