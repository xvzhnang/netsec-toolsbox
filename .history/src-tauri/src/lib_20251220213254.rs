...
pub fn run() {
  tauri::Builder::default()
    .setup(|app| {
      ...
      // 注册 AI Gateway 服务
      let ai_gateway_service: service::ServiceHandle = Arc::new(Mutex::new(
      ...
      service_manager.lock().unwrap()
        .register(ai_gateway_service)
        .map_err(|e| {
          log::error!("注册 AI Gateway 服务失败: {}", e);
          std::io::Error::new(
            std::io::ErrorKind::Other,
            format!("注册 AI Gateway 服务失败: {}", e),
          )
        })?;
      
      // 启动监控线程
      service_manager.lock().unwrap().start_monitoring();
      
      log::info!("[ServiceManager] 已初始化并启动监控");
      
      // 将 ServiceManager 添加到应用状态
      // 注意：在 Tauri 2.x 中，manage 需要在 setup 闭包内调用
      app.state().manage(service_manager);
