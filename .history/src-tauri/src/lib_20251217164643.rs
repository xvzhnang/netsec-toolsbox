...
/// 提取 EXE 图标（Windows）
/// 使用 SHGetFileInfoW 或 ExtractIconExW 获取 HICON，然后在内存中转换为图片数据
#[cfg(target_os = "windows")]
fn extract_exe_icon(file_path: &str) -> Result<DynamicImage, String> {
  ...
  // 使用 SHGetFileInfoW 获取文件图标（更简单高效）
  unsafe {
    let mut file_info = windows::Win32::UI::Shell::SHFILEINFOW {
      hIcon: HICON::default(),
      iIcon: 0,
      dwAttributes: 0,
      szDisplayName: [0; 260],
    ...
    let result = SHGetFileInfoW(
      ...
      0,
      &mut file_info,
      mem::size_of::<windows::Win32::UI::Shell::SHFILEINFOW>() as u32,
      SHGFI_ICON | SHGFI_LARGEICON,
    );
    
    let hicon = if result != 0 && !file_info.hIcon.is_invalid() {
      file_info.hIcon
    } else {
      // 如果 SHGetFileInfoW 失败，尝试使用 ExtractIconExW
