...
fn extract_exe_icon(file_path: &str) -> Result<DynamicImage, String> {
  ...
  // 使用绝对路径的字符串表示，正确处理 UTF-8 编码
  let p = clean_path.replace('\\', "\\\\");
  
  // 使用 PowerShell/.NET 提取图标
  // 使用 UTF-8 编码处理路径，确保中文字符正确传递
  let script = format!(r#"
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$ErrorActionPreference = 'Stop'
try {{
  $filePath = [System.Text.Encoding]::UTF8.GetString([System.Text.Encoding]::Default.GetBytes('{}'))
...
fn extract_lnk_icon(lnk_path: &str) -> Result<DynamicImage, String> {
  ...
  // 使用绝对路径的字符串表示，正确处理 UTF-8 编码
  // 将路径转换为 PowerShell 可以正确处理的格式
  // 使用 [System.Text.Encoding]::UTF8 确保中文字符正确处理
  let p = clean_path.replace('\\', "\\\\");
...
$ErrorActionPreference = 'Stop'
try {{
  $lnkPath = [System.Text.Encoding]::UTF8.GetString([System.Text.Encoding]::Default.GetBytes('{}'))
