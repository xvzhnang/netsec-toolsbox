...
pub fn open_file_dialog(params: OpenFileDialogParams) -> Result<Option<String>, String> {
  #[cfg(target_os = "windows")]
  {
    ...
    // 注意：这是一个简化的实现，可能不如原生对话框功能完整
    let filter_str = if let Some(ref filters) = params.filters {
      if !filters.is_empty() {
        let ext_list: Vec<String> = filters.iter()
    ...
    let default_path_str = if let Some(ref path) = params.default_path {
      format!(" -InitialDirectory \"{}\"", path.replace('\\', "\\\\"))
    } else {
      String::new()
    };
