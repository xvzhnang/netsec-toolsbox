...
fn extract_exe_icon(file_path: &str) -> Result<DynamicImage, String> {
  ...
  // 使用 Path::display() 确保路径格式正确，然后转义反斜杠（与参考代码一致）
  let p = path.display().to_string().replace('\\', "\\\\");
  
  // 使用 PowerShell/.NET 提取图标（完全按照参考代码格式）
  let script = format!(r#"
try {{
  Add-Type -AssemblyName System.Drawing
  $ic = [System.Drawing.Icon]::ExtractAssociatedIcon('{}')
  if ($ic -ne $null) {{ $bmp = $ic.ToBitmap(); $ms = New-Object System.IO.MemoryStream; $bmp.Save($ms,[System.Drawing.Imaging.ImageFormat]::Png); [Convert]::ToBase64String($ms.ToArray()) }}
...
fn extract_lnk_icon(lnk_path: &str) -> Result<DynamicImage, String> {
  ...
  // 使用 Path::display() 确保路径格式正确，然后转义反斜杠
  let path = Path::new(clean_path);
  let p = path.display().to_string().replace('\\', "\\\\");
  
  // 使用 PowerShell/.NET 提取 LNK 图标（参考代码格式）
  // 优先使用快捷方式的 IconLocation，如果没有则使用目标路径
  let script = format!(r#"
try {{
  $s=(New-Object -ComObject WScript.Shell).CreateShortcut('{}');
  $icon=$s.IconLocation; if([string]::IsNullOrEmpty($icon)){{ $icon = $s.TargetPath }}
  $parts = $icon -split ','
