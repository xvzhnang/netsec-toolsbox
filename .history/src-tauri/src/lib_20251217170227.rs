...
/// 提取 EXE 图标（Windows）
/// 使用 ExtractIconExW 获取 HICON，然后在内存中转换为图片数据
/// 不生成文件，直接在内存中处理
#[cfg(target_os = "windows")]
fn extract_exe_icon(file_path: &str) -> Result<DynamicImage, String> {
  ...
  // 使用 ExtractIconExW 提取图标（优先使用大图标）
  unsafe {
    let _hmod = LoadLibraryW(PCWSTR(wide_path.as_ptr()))
    ...
    let hicon = if count > 0 && !large_icon.is_invalid() {
      large_icon
    } else if count > 0 && !small_icon.is_invalid() {
      small_icon
    } else {
    ...
    // 使用通用的 HICON 转图片函数（在内存中转换，不生成文件）
    hicon_to_image(hicon)
  }
}

/// 提取 LNK 图标（Windows）
/// 解析快捷方式获取目标路径，然后提取目标文件的图标
/// 如果解析失败，尝试直接从 LNK 文件提取图标
#[cfg(target_os = "windows")]
fn extract_lnk_icon(lnk_path: &str) -> Result<DynamicImage, String> {
  use windows::Win32::System::Com::{CoInitialize, CoUninitialize, CoCreateInstance, CLSCTX_INPROC_SERVER};
  use windows::Win32::UI::Shell::{IShellLinkW, CLSID_ShellLink};
  ...
  unsafe {
    ...
    let result = (|| -> Result<DynamicImage, String> {
      ...
      let persist_file: windows::Win32::System::Com::IPersistFile = shell_link.cast()
        .map_err(|e| format!("获取 IPersistFile 接口失败: {:?}", e))?;
      ...
      if target_str.is_empty() {
        // 如果无法获取目标路径，尝试直接从 LNK 文件提取图标
        return extract_exe_icon(lnk_path);
      }
      
      // 提取目标文件的图标
      extract_exe_icon(&target_str)
    })();
