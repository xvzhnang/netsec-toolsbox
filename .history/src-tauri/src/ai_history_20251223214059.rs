...
pub fn ai_history_update_message(
    ...
    let (sid, mid, _role, old_text, old_ts, old_model, old_usage_json) = match row {
        Some(v) => v,
        None => return Ok(()),
    };
...
#[tauri::command]
pub fn ai_history_set_current_session(session_id: String) -> Result<(), String> {
    let conn = open_db()?;
    ...
    set_current_session(&conn, &session_id)?;
    Ok(())
}
