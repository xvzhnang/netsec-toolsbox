...
pub fn run() {
    ...
    let app = tauri::Builder::default()
        ...
        .manage(splashscreen::FrontendReady::default())
        ...
        .setup(|app| {
            ...
            // 优化启动时间：等待前端首帧 ready 后再启动重服务，避免抢占 WebView 首屏资源
            let app_handle = app.handle().clone();
            std::thread::spawn(move || {
                // 关键优化：减少延迟到 200ms，加快启动速度
                // 使用 catch_unwind 捕获 panic，避免启动线程崩溃导致应用无法启动
                let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    log::info!("[启动优化] 正在等待前端 UI 就绪 (超时时间: 60s)...");
                    let _ = app_handle
                        .state::<splashscreen::FrontendReady>()
                        .wait_ready_timeout(std::time::Duration::from_secs(60));
                    
                    log::info!("[启动优化] 前端 UI 已就绪，开始后台服务启动流程");
    ...
    app.run(|app_handle, event| {
        match event {
            tauri::RunEvent::ExitRequested { .. } => {
                ...
                let _ = mkdocs_server::stop_mkdocs(mkdocs_state.inner());
            }
