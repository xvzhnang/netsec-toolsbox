...
impl ServiceManager {
    ...
    pub fn new() -> Self {
        Self {
            ...
            monitoring: Arc::new(parking_lot::Mutex::new(false)),
            event_bus: Arc::new(parking_lot::Mutex::new(EventBus::new())),
            circuit_breakers: Arc::new(parking_lot::Mutex::new(HashMap::new())),
            metrics: Arc::new(std::sync::Mutex::new(MetricsCollector::new())),
            restart_policy: RestartPolicy::default(),
            recovery: Arc::new(parking_lot::Mutex::new(HashMap::new())),
            monitoring_controller: Some(Arc::new(ChannelLoopController::new())),
            watchdog: Arc::new(parking_lot::Mutex::new(Some(Watchdog::new()))),
        }
    }
    ...
    pub fn get_all_status(&self) -> ServiceStatusListDTO {
        ...
        for (_, service) in services.iter() {
            let service_guard = crate::utils::lock_or_recover_std(service.as_ref(), "ServiceHandle");
    ...
    pub fn start_service(&self, id: &str) -> Result<(), String> {
        ...
        let mut service_guard = crate::utils::lock_or_recover_std(service.as_ref(), "ServiceHandle");
    ...
    pub fn start_monitoring(&self) {
        ...
        *monitoring = true;
        ...
        // ✅ 工程原则：使用 Channel 驱动循环
        let controller = self.monitoring_controller.as_ref()
            .map(Arc::clone)
        ...
        // ✅ 启动心跳发送线程（每 10 秒发送一次 Tick）
        let tick_tx = controller.sender().clone();
        thread::spawn(move || {
            loop {
                thread::sleep(Duration::from_secs(10));
                if tick_tx.send(LoopMessage::Tick).is_err() {
                    break; // 接收端已关闭
                }
            }
        });
        ...
        thread::spawn(move || {
            ...
            loop {
                ...
                let service_ids: Vec<(String, Arc<ServiceHandle>)> = {
                    let services_guard =
                        crate::utils::lock_or_recover_std(services.as_ref(), "ServiceManager.services");
                    services_guard
                        .iter()
                ...
                for (id, service) in service_ids {
                    ...
                    let (should_skip, needs_restart, needs_start_tracking) = {
                        ...
                        if entry.paused {
                            (true, false, false)
                        } else if entry.restart_in_progress {
                            (true, false, false)
                        } else {
                    ...
                    if needs_restart {
                        restarts_due.push((id.clone(), service));
                        continue;
                    }
                    ...
                    match health_result {
                        ...
                        if pause_service {
                            ...
                            stops_due.push((id.clone(), service));
                            continue;
                        }
                ...
                for (id, service) in stops_due {
                    ...
                    thread::spawn(move || {
                        let mut s =
                            crate::utils::lock_or_recover_std(service_clone.as_ref(), "ServiceHandle");
                        let from = s.state();
                ...
                // ✅ 工程原则：心跳检查（Watchdog）
                // 如果心跳超时（30秒），说明循环可能卡死，记录警告
                if !heartbeat_clone.is_alive(30000) {
                    warn!("[ServiceManager] 监控线程心跳超时，可能卡死");
                }
                }
            }));
