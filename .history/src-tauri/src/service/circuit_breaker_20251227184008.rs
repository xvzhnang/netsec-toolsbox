...
impl CircuitBreaker {
    ...
    pub fn opened_elapsed(&self) -> Option<Duration> {
        let opened_at =
            *crate::utils::lock_or_recover_std(self.opened_at.as_ref(), "CircuitBreaker.opened_at");
        opened_at.map(|t| t.elapsed())
    }

    pub fn reset(&self) {
        let mut state_guard =
            crate::utils::lock_or_recover_std(self.state.as_ref(), "CircuitBreaker.state");
        *state_guard = CircuitBreakerState::Closed;
        drop(state_guard);

        let mut failures = crate::utils::lock_or_recover_std(
        ...
        *failures = 0;
        ...
        let mut successes = crate::utils::lock_or_recover_std(
    ...
    pub fn can_execute(&self) -> bool {
        let state = *crate::utils::lock_or_recover_std(self.state.as_ref(), "CircuitBreaker.state");
        
        match state {
            CircuitBreakerState::Closed => true,
            CircuitBreakerState::Open => {
                // 检查是否超时，进入半开状态
                if let Some(opened_at) = *crate::utils::lock_or_recover_std(
                    self.opened_at.as_ref(),
                    "CircuitBreaker.opened_at",
                ) {
                    if opened_at.elapsed() >= self.config.timeout {
                        let mut state_guard = crate::utils::lock_or_recover_std(
                            self.state.as_ref(),
    ...
    pub fn record_success(&self) {
        ...
        let mut history = crate::utils::lock_or_recover_std(
            ...
            CircuitBreakerState::HalfOpen => {
                ...
                *successes += 1;
                ...
                if *successes >= self.config.success_threshold {
                    *state_guard = CircuitBreakerState::Closed;
                    *failures = 0;
                    let mut opened_at = crate::utils::lock_or_recover_std(
                        self.opened_at.as_ref(),
                        "CircuitBreaker.opened_at",
                    );
                    *opened_at = None;
    ...
    pub fn failure_rate(&self) -> f64 {
        let history = crate::utils::lock_or_recover_std(
            self.request_history.as_ref(),
            "CircuitBreaker.request_history",
        );
        let now = Instant::now();
...
impl RateLimiter {
    ...
    pub fn allow(&self) -> bool {
        let mut tokens = crate::utils::lock_or_recover_std(self.tokens.as_ref(), "RateLimiter.tokens");
        let mut last_update =
            crate::utils::lock_or_recover_std(self.last_update.as_ref(), "RateLimiter.last_update");
        let now = Instant::now();
