use crate::utils::ChannelLoopController;
...
pub struct ServiceManager {
    ...
    status_cache: Arc<StdMutex<HashMap<String, ServiceStatusDTO>>>,
...
impl ServiceManager {
    ...
    pub fn register(&self, service: ServiceHandle) -> Result<(), String> {
        ...
        let (id, dto) = {
            let s = crate::utils::lock_or_recover_std(service.as_ref(), "ServiceHandle");
            (s.id().to_string(), ServiceStatusDTO::from_service(&*s))
        };
        ...
        {
            let mut cache = crate::utils::lock_or_recover_std(
                self.status_cache.as_ref(),
                "ServiceManager.status_cache",
            );
            cache.insert(id.clone(), dto);
        }
    ...
    pub fn get_all_status(&self) -> ServiceStatusListDTO {
        ...
        let handles: Vec<(String, ServiceHandle)> = {
            let services = crate::utils::lock_or_recover_std(
                self.services.as_ref(),
                "ServiceManager.services",
            );
        ...
        for (id, service) in handles {
            let dto = match service.try_lock() {
                Ok(service_guard) => {
                ...
                Err(_) => {
                    let cache = crate::utils::lock_or_recover_std(
                        self.status_cache.as_ref(),
                        "ServiceManager.status_cache",
                    );
                    cache.get(&id).cloned().unwrap_or_else(|| ServiceStatusDTO {
                        id: id.clone(),
                        name: id.clone(),
    ...
    pub fn get_status(&self, id: &str) -> Option<ServiceStatusDTO> {
        let services =
            crate::utils::lock_or_recover_std(self.services.as_ref(), "ServiceManager.services");
        let service = services.get(id).cloned();
        drop(services);
        ...
        if let Some(service) = service {
            match service.try_lock() {
                Ok(service_guard) => {
                    ...
                    cache.insert(id.to_string(), dto.clone());
                    Some(dto)
                }
                Err(_) => {
                    let cache = crate::utils::lock_or_recover_std(
    ...
    pub fn start_monitoring(&self) {
        ...
        *monitoring = true;
        ...
        let tick_monitoring_flag = Arc::clone(&self.monitoring);
        ...
        thread::spawn(move || loop {
            ...
            let continue_flag = tick_monitoring_flag.lock();
            if !*continue_flag {
                break;
            }
            ...
            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                ...
                crate::utils::run_channel_loop(
                    ...
                    for (id, service) in service_ids {
                        ...
                        // ✅ 关键修复：快速检查恢复状态，立即释放锁
                        let (should_skip, needs_restart, _needs_start_tracking) = {
