        ...
        *monitoring = true;
        ...
        thread::spawn(move || {
            ...
            loop {
                ...
                let now = Instant::now();
                let mut restarts_due: Vec<(String, Arc<ServiceHandle>)> = Vec::new();
                let mut stops_due: Vec<(String, Arc<ServiceHandle>)> = Vec::new();
                        ...
                        if let Some((from, to)) = state_change {
                            if let Err(e) = service_guard.set_state(to) {
                                warn!("[ServiceManager] 服务 {} 状态切换失败: {}", id, e);
                                    ...
                                    {
                                    let metrics_guard: std::sync::MutexGuard<'_, MetricsCollector> = crate::utils::lock_or_recover_std(
                                        metrics.as_ref(),
                                        "ServiceManager.metrics",
                                    );
