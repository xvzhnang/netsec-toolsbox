...
fn process_icon_to_base64(img: DynamicImage, size: u32) -> Result<String, String> {
  ...
  let mut png_data = Vec::new();
  {
    ...
    encoder.write_image(&rgba, size, size, image::ColorType::Rgba8)
...
fn extract_icon_from_file(file_path: String, tool_type: Option<String>) -> Result<String, String> {
  ...
  if let Ok(img) = image::load_from_memory(&general_purpose::STANDARD.decode(
    ...
    let mut png_data = Vec::new();
    {
      ...
      encoder.encode(&rgba, 160, 160, image::ColorType::Rgba8)
        .map_err(|e| format!("PNG 编码失败: {}", e))?;
    }
...
fn extract_exe_icon(file_path: &str) -> Result<DynamicImage, String> {
  ...
  use windows::Win32::UI::Shell::{ExtractIconW, DestroyIcon};
  use windows::Win32::Graphics::Gdi::{GetIconInfo, GetObjectW, BITMAP, GetDIBits, CreateDIBSection, GetDC, ReleaseDC, DeleteObject, BITMAPINFO, BITMAPINFOHEADER, DIB_RGB_COLORS};
  ...
  unsafe {
    ...
    let hicon = ExtractIconW(hmod, PCWSTR(wide_path.as_ptr()), 0)
      .ok()
    ...
    if GetObjectW(icon_info.hbmColor, std::mem::size_of::<BITMAP>() as i32, Some(&mut bmp as *const _ as *mut _)).is_err() {
      DestroyIcon(hicon);
    ...
    // 创建一个简单的占位图标
    Ok(DynamicImage::ImageRgba8(image::RgbaImage::from_pixel(width, height, image::Rgba([128, 128, 128, 255]))))
  }
}
