...
/// 在 Windows 上使用 CMD 打开终端并执行命令
#[cfg(target_os = "windows")]
fn launch_in_terminal_windows_cmd(working_dir: &Path, command: &str) -> Result<(), String> {
...
fn launch_python_tool(exec_path: &str, args: Option<Vec<String>>) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // Windows CMD 命令构建（无需转义双引号，只需用引号包裹路径）
        let mut command = format!("{} \"{}\"", python_cmd, exec_path);
        if let Some(ref args_vec) = args {
            for arg in args_vec {
                if arg.contains(' ') {
                    command.push_str(&format!(" \"{}\"", arg));
        ...
        launch_in_terminal_windows_cmd(&working_dir, &command)?;
    }
