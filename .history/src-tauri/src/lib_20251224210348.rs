...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let manager = app.state::<std::sync::Mutex<service::ServiceManager>>();
            let event_bus = {
                let manager_guard =
                    crate::utils::lock_or_recover(&*manager, "ServiceManager (setup)");
                manager_guard.event_bus()
            };
            {
                let mut bus_guard =
            ...
            let manager_state = app.state::<std::sync::Mutex<service::ServiceManager>>();
            let manager_clone = manager_state.inner().clone();
            std::thread::spawn(move || {
                ...
                let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    ...
                    let manager = crate::utils::lock_or_recover(&manager_clone, "ServiceManager (async start)");
                    if let Err(e) = manager.start_service("ai-gateway") {
                        log::warn!("[启动优化] 后台启动 AI Gateway 服务失败: {}", e);
                    } else {
                        log::info!("[启动优化] AI Gateway 服务已在后台启动");
                    }
                    
                    // 启动监控线程
                    drop(manager);
                    let manager = crate::utils::lock_or_recover(&manager_clone, "ServiceManager (start monitoring)");
