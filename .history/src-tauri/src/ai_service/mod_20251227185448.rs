    ...
    match start_result {
        Ok(results) => {
            ...
            loop {
                ...
                if state != crate::ai_service::pool::GatewayState::Unavailable {
                    return Ok("连接池已初始化".to_string());
                }

                if Instant::now() >= deadline {
                    let reason = "No worker available after 10s".to_string();
                    {
                        let pool_guard =
                            crate::utils::lock_or_recover(pool.as_ref(), "GatewayPool");
                        pool_guard.mark_startup_failed(reason.clone());
                    }
                    return Err(format!("初始化连接池失败: {}", reason));
                }
