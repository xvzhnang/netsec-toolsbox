...
fn schedule_restart_for_worker(worker: Arc<Mutex<GatewayWorker>>, signal: HealthSignal) {
    // 关键修复：获取 user_stopped 标志，如果服务被用户停止，不进行重启
    let user_stopped = {
        // 通过 worker 获取 pool 的 user_stopped 标志
        // 由于 worker 没有直接引用 pool，我们需要通过其他方式检查
        // 暂时通过检查 worker 状态来判断
        let wg = crate::utils::lock_or_recover(worker.as_ref(), "GatewayWorker");
        matches!(wg.status(), WorkerState::Dead | WorkerState::Zombie)
    };
