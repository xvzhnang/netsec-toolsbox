...
impl GatewayPool {
    ...
    pub fn get_gateway_state(&self) -> GatewayState {
        ...
        for worker in &self.workers {
            ...
            match state {
                ...
                WorkerState::BusyStreaming | WorkerState::BusyBlocked => busy_count += 1,
                WorkerState::Unhealthy | WorkerState::Dead | WorkerState::Restarting => unhealthy_count += 1,
                WorkerState::Degraded => {
                    idle_count += 1; // Degraded 仍可接受请求
                    unhealthy_count += 1;
                }
                _ => {}
            }
    ...
    pub fn select_worker(&mut self, client_id: Option<&str>) -> Option<Arc<Mutex<GatewayWorker>>> {
        ...
        if let Some(cid) = client_id {
            ...
            let worker_guard = worker.lock().unwrap();
            let state = worker_guard.status();
            if state.is_available() && worker_guard.is_healthy() {
        ...
        for worker in &self.workers {
            ...
            let effective_weight = metrics.effective_weight(1.0 + metrics.active_requests as f64);
