...
pub fn start_ai_service(state: State<AIServiceState>) -> Result<String, String> {
    ...
    match child.try_wait() {
        Ok(Some(status)) => {
            ...
            return Err(format!("AI Gateway 服务进程立即退出，退出状态: {:?}", status));
        }
        Ok(None) => {
            ...
            match child.try_wait() {
                Ok(Some(status)) => {
                    ...
                    return Err(format!("AI Gateway 服务进程在启动后退出，退出状态: {:?}", status));
                }
                Ok(None) => {
...
fn get_models_config_path() -> std::path::PathBuf {
    ...
    log::debug!("get_models_config_path: 项目根目录: {}, 配置文件路径: {}", 
                base_dir.display(), config_path.display());
