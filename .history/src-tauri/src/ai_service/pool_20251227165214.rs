...
static UNAVAILABLE_MODELS: OnceLock<parking_lot::Mutex<BTreeSet<String>>> = OnceLock::new();
...
fn try_graceful_shutdown(port: u16) {
...
fn wait_port_free(port: u16, timeout: Duration) -> bool {
    ...
    const MAX_CHECKS: u32 = 50; // 限制最大检查次数，避免无限循环
    ...
    loop {
        ...
        if start.elapsed() >= timeout || check_count >= MAX_CHECKS {
            return false;
        }
        // 优化：使用指数退避，减少 CPU 占用
        let sleep_ms = if check_count < 5 { 50 } else { 100 };
        thread::sleep(Duration::from_millis(sleep_ms));
    }
}

#[cfg(target_os = "windows")]
fn port_to_listening_pids(port: u16) -> Vec<u32> {
...
#[cfg(target_os = "windows")]
fn kill_listening_pid_on_port(port: u16) -> bool {
    let pids = port_to_listening_pids(port);
    if pids.is_empty() {
        return false;
    }
...
impl GatewayWorker {
    ...
    pub fn active_requests(&self) -> u32 {
        crate::utils::lock_or_recover_std(self.metrics.as_ref(), "GatewayWorker.metrics")
            .active_requests
    }
...
impl GatewayWorker {
    pub fn new(id: usize, port: u16) -> Self {
        ...
        Self {
            ...
            restart_budget: RestartBudget::new(Duration::from_secs(300), 6),
    ...
    pub fn is_healthy(&self) -> bool {
        let state = self.status();
            let metrics = crate::utils::lock_or_recover_std(self.metrics.as_ref(), "GatewayWorker.metrics");
        
        state != WorkerState::Dead 
            && state != WorkerState::FailedPermanent
            && state != WorkerState::Disabled
            && state != WorkerState::Zombie
...
fn schedule_restart_for_worker(worker: Arc<Mutex<GatewayWorker>>, signal: HealthSignal) {
    let (delay, worker_id) = {
        let mut wg = crate::utils::lock_or_recover_std(worker.as_ref(), "GatewayWorker");
...
impl GatewayPool {
    ...
    pub fn select_worker(&mut self, client_id: Option<&str>) -> Option<Arc<Mutex<GatewayWorker>>> {
        ...
        if let Some(cid) = client_id {
            ...
            let worker = &self.workers[hash];
                let mut worker_guard =
                    crate::utils::lock_or_recover_std(worker.as_ref(), "GatewayWorker");
        ...
        for worker in &self.workers {
            let worker_guard = crate::utils::lock_or_recover_std(worker.as_ref(), "GatewayWorker");
            let state = worker_guard.status();
        ...
        if let Some(worker) = best_worker.as_ref() {
            ...
            {
                let active_requests = crate::utils::lock_or_recover_std(
                    worker_guard.metrics.as_ref(),
                    "GatewayWorker.metrics",
                )
                .active_requests;
    ...
    pub fn start_all(&self) -> Result<Vec<String>, String> {
        ...
        let workers_to_start: Vec<(usize, u16)> = {
            ...
            for (idx, worker) in self.workers.iter().enumerate() {
                ...
                if worker_guard.process.is_none() {
                    workers_to_start.push((worker_guard.id, worker_guard.port));
                }
            }
            workers_to_start
        };
        
        // 关键优化：逐个启动 Worker，每次只持有单个 Worker 的锁，避免长时间阻塞
        for (worker_id, _port) in workers_to_start {
            ...
            // 关键优化：立即释放 Worker 锁，避免阻塞下一个 Worker 的启动
            drop(worker_guard);
        }
    ...
    pub fn stop_all(&self) -> Result<Vec<String>, String> {
        ...
        for worker in &self.workers {
            ...
            if let Some(mut child) = worker_guard.process.take() {
                ...
                try_graceful_shutdown(worker_port);
    ...
    fn start_worker(worker: &mut GatewayWorker) -> Result<String, String> {
        ...
        // 不等待完全就绪，避免阻塞启动流程，让健康检查线程处理后续状态转换
        if !saw_ready {
            let current_state = worker.status();
            if current_state == WorkerState::Init {
                worker.set_state(WorkerState::Ready);
                log::debug!("[Gateway Pool] Worker-{} 设置为 Ready 状态，后台继续初始化（快速启动模式）", worker.id);
            }
        }
    ...
    pub fn get_models_cached(&self) -> Result<Vec<String>, String> {
        ...
        {
            let cache_guard =
                crate::utils::lock_or_recover_std(self.model_cache.as_ref(), "GatewayPool.model_cache");
