...
impl WorkerState {
    ...
    pub fn from_u8(value: u8) -> Self {
        match value {
            ...
            4 => WorkerState::Unhealthy,
            5 => WorkerState::Restarting,
            6 => WorkerState::Dead,
            _ => WorkerState::Dead,
        }
    }
    ...
    pub fn is_healthy(&self) -> bool {
        matches!(self, WorkerState::Idle | WorkerState::Busy | WorkerState::Degraded)
    }

    /// 检查是否允许健康检查（Busy 时只允许 L0/L1）
    pub fn allows_health_check(&self, level: HealthCheckLevel) -> bool {
        match self {
            WorkerState::Busy => matches!(level, HealthCheckLevel::L0 | HealthCheckLevel::L1),
            _ => true,
        }
    }
}
...
pub struct GatewayWorker {
    ...
    /// 总错误数
    pub total_errors: Arc<Mutex<u64>>,
    /// 是否熔断（超时或错误过多）
    pub circuit_breaker_open: Arc<Mutex<bool>>,
    /// 熔断开始时间
...
impl GatewayWorker {
    pub fn new(id: usize, port: u16) -> Self {
        Self {
            ...
            port,
            process: None,
            state: Arc::new(AtomicU8::new(WorkerState::Dead.to_u8())),
    ...
    pub fn should_attempt_recovery(&self) -> bool {
        let breaker_open = *self.circuit_breaker_open.lock().unwrap();
        if !breaker_open {
            return false;
        }
        
        // 如果熔断器打开超过 30 秒，尝试恢复
        if let Some(opened_at) = *self.circuit_breaker_opened_at.lock().unwrap() {
    ...
    pub fn ping_url(&self) -> String {
        format!("http://127.0.0.1:{}/ping", self.port)
    }
    ...
    pub fn record_success(&self, latency_ms: u64) {
        ...
        if metrics.avg_latency_ms == 0 {
            ...
            metrics.avg_latency_ms = (metrics.avg_latency_ms * 7 + latency_ms * 3) / 10;
        }
    }
...
impl GatewayPool {
    ...
    /// 选择一个可用的 Worker（负载均衡：轮询）
    pub fn select_worker(&mut self) -> Option<Arc<Mutex<GatewayWorker>>> {
        ...
        loop {
            ...
            if worker_guard.is_healthy() 
                && worker_guard.status != WorkerStatus::Busy 
                && worker_guard.active_requests < 10 {
                // 更新轮询索引
                self.current_index = (self.current_index + 1) % self.pool_size;
                return Some(Arc::clone(worker));
            }

            // 如果所有 Worker 都不可用，尝试选择最不忙的
            if attempts >= self.pool_size {
                // 选择错误最少的 Worker
                ...
                for w in &self.workers {
                    ...
                    if wg.status != WorkerStatus::Dead {
                        let score = wg.total_errors * 100 + wg.active_requests as u64;
                        if score < best_score {
                            best_score = score;
                            best_worker = Some(Arc::clone(w));
                        }
                    }
                }
            ...
            if worker_guard.process.is_none() {
                match Self::start_worker(&mut worker_guard) {
                    ...
                    Err(e) => {
                        ...
                        worker_guard.status = WorkerStatus::Dead;
                    }
                }
            }
        }
    ...
    fn start_worker(worker: &mut GatewayWorker) -> Result<String, String> {
        ...
        *worker.last_health_check.lock().unwrap() = Some(Instant::now());
        *worker.circuit_breaker_open.lock().unwrap() = false;
    ...
    pub fn health_check_worker(&self, worker_id: usize) -> bool {
        ...
        match client.get(&health_url).send() {
            Ok(response) => {
                if response.status() == StatusCode::OK {
                    ...
                    worker_guard.consecutive_failures = 0;
                    worker_guard.status = WorkerStatus::Idle;
        ...
        // 健康检查失败
        worker_guard.last_health_check = Some(Instant::now());
        worker_guard.consecutive_failures += 1;
        
        // 如果连续失败 5 次，打开熔断器
        if worker_guard.consecutive_failures >= 5 {
            worker_guard.circuit_breaker_open = true;
            worker_guard.circuit_breaker_opened_at = Some(Instant::now());
            worker_guard.status = WorkerStatus::Unhealthy;
            log::warn!("[Gateway Pool] Worker-{} 连续失败 {} 次，打开熔断器", worker_id, worker_guard.consecutive_failures);
        } else {
            worker_guard.status = WorkerStatus::Unhealthy;
        }
    ...
    pub fn health_check_all(&self) {
        for (idx, worker) in self.workers.iter().enumerate() {
            ...
            if worker_guard.status != WorkerStatus::Dead {
                drop(worker_guard);
                    ...
                    match client.get(&health_url).send() {
                        Ok(response) => {
                            ...
                            if response.status() == StatusCode::OK {
                                ...
                                wg.last_success = Some(Instant::now());
                                wg.consecutive_failures = 0;
                                wg.status = WorkerStatus::Idle;
                                wg.circuit_breaker_open = false;
                                wg.circuit_breaker_opened_at = None;
                            } else {
                                wg.last_health_check = Some(Instant::now());
                                wg.consecutive_failures += 1;
                                if wg.consecutive_failures >= 5 {
                                    wg.circuit_breaker_open = true;
                                    wg.circuit_breaker_opened_at = Some(Instant::now());
                                    wg.status = WorkerStatus::Unhealthy;
                                    log::warn!("[Gateway Pool] Worker-{} 连续失败 {} 次，打开熔断器", idx, wg.consecutive_failures);
                                }
                            }
                        }
                        Err(e) => {
                            ...
                            if wg.consecutive_failures >= 5 {
                                ...
                                wg.circuit_breaker_opened_at = Some(Instant::now());
                                wg.status = WorkerStatus::Unhealthy;
                                log::warn!("[Gateway Pool] Worker-{} 健康检查失败，打开熔断器: {}", idx, e);
                            }
                        }
                    }
                }
    ...
    pub fn forward_request(&mut self, method: &str, path: &str, body: Option<&[u8]>, headers: Option<&[(&str, &str)]>) -> Result<(StatusCode, Vec<u8>), String> {
        ...
        for attempt in 0..max_retries {
            ...
            {
                let mut wg = worker.lock().unwrap();
                wg.status = WorkerStatus::Busy;
                wg.active_requests += 1;
                wg.total_requests += 1;
            }
            ...
            match result {
                ...
                Err(e) => {
                    ...
                    // 记录失败
                    {
                        ...
                        // 如果超时或连续失败，打开熔断器
                        if elapsed >= timeout || wg.consecutive_failures >= 5 {
                            wg.circuit_breaker_open = true;
                            wg.circuit_breaker_opened_at = Some(Instant::now());
                            wg.status = WorkerStatus::Unhealthy;
                            log::warn!("[Gateway Pool] Worker-{} 打开熔断器", worker_id);
                        }
                    }
