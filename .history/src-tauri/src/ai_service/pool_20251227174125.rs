...
use crate::utils::{
    find_free_port, is_port_free, kill_listening_pid_on_port, try_graceful_shutdown, wait_port_free,
};
...
impl GatewayWorker {
    ...
    pub fn should_mark_fatal_for_restart(&self, now: Instant) -> bool {
    ...
    pub fn record_failure(&mut self, is_timeout: bool) -> (u32, u32) {
        let mut metrics = crate::utils::lock_or_recover(self.metrics.as_ref(), "GatewayWorker.metrics");
    ...
    pub fn record_success(&self) {
        ...
        metrics.recent_fail_rate = metrics.recent_fail_rate * 0.8; // Decay
    ...
    pub fn is_healthy(&self) -> bool {
        !matches!(
            self.status(), 
            WorkerState::Unhealthy | WorkerState::Dead | WorkerState::Zombie | WorkerState::FailedPermanent | WorkerState::Disabled
        )
    }
}
...
impl GatewayPool {
    ...
    pub fn health_check_worker(&self, worker_id: usize) -> bool {
        ...
        let heartbeat_ok = {
            ...
            if let Some(last_heartbeat) = metrics.last_heartbeat {
                last_heartbeat.elapsed() < Duration::from_secs(60) // 心跳 60秒内有效
            } else {
    ...
    pub fn get_models_cached(&self) -> Result<Vec<String>, String> {
        ...
        let models: Vec<String> =
            if let Some(data_array) = data.get("data").and_then(|d: &serde_json::Value| d.as_array()) {
                data_array
                    .iter()
