...
impl GatewayPool {
    ...
    fn start_worker(worker: &mut GatewayWorker) -> Result<String, String> {
        ...
        // 优化启动时间：减少初始等待，允许后台异步就绪
        let start = Instant::now();
        let max_wait = Duration::from_millis(800); // 从 1500ms 减少到 800ms
        let mut saw_ready = false;
        let mut check_count = 0;
        const MAX_CHECKS: u32 = 16; // 最多检查 16 次 (800ms / 50ms)
        ...
        // 如果未就绪，设置为 Ready 状态，允许后台继续初始化
        if !saw_ready && worker.status() == WorkerState::Init {
            worker.set_state(WorkerState::Ready);
            log::debug!("[Gateway Pool] Worker-{} 设置为 Ready 状态，后台继续初始化", worker.id);
        }
