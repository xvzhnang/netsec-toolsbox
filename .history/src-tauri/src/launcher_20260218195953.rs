...
#[cfg(target_os = "windows")]
fn launch_in_terminal_windows_pwsh(working_dir: &Path, command: &str) -> Result<(), String> {
    // 使用 Start-Process 启动新的 PowerShell 窗口，并执行命令
...
fn launch_python_tool(exec_path: &str, args: Option<Vec<String>>) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // 构建 Python 命令
        // 注意：这里使用单引号包裹路径，以避免 PowerShell 解析问题
        // 如果路径中包含单引号，需要转义
        let script_path = exec_path.replace('\'', "''");
        let mut command = format!("{} '{}'", python_cmd, script_path);
        
        if let Some(ref args_vec) = args {
            for arg in args_vec {
                // 参数处理：始终使用单引号包裹，并转义内部单引号
                command.push_str(&format!(" '{}'", arg.replace('\'', "''")));
            }
        }
        launch_in_terminal_windows_pwsh(&working_dir, &command)?;
    }
