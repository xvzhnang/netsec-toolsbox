...
fn extract_exe_icon(file_path: &str) -> Result<DynamicImage, String> {
  ...
  use windows::Win32::Graphics::Gdi::{
    ...
    SelectObject, GetDIBits, CreateDIBSection, DeleteObject, BITMAPINFO, BITMAPINFOHEADER, DIB_RGB_COLORS, HBITMAP, HGDIOBJ
  };
  ...
  unsafe {
    ...
    // 创建 DIB section
    let mut bits_ptr: Option<*mut std::ffi::c_void> = None;
    ...
    // 将图标位图复制到 DIB
    let _old_icon = SelectObject(hdc_mem, icon_info.hbmColor);
    
    // 获取位图数据
    let stride = (width * 4) as i32; // RGBA = 4 bytes per pixel
    ...
    match image::RgbaImage::from_raw(width, height, rgba_buffer) {
      ...
      None => Err("无法从位图数据创建图像".to_string()),
    }
  }
}
