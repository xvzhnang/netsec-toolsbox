...
fn extract_exe_icon(file_path: &str) -> Result<DynamicImage, String> {
  ...
  // 移除 Windows 长路径前缀 (\\?\)，因为 ExtractIconExW 可能不支持
  ...
  // 转换路径为宽字符串
  let wide_path: Vec<u16> = OsStr::new(clean_path).encode_wide().chain(Some(0)).collect();
...
/// 提取 LNK 图标（Windows）
/// ExtractIconExW 会自动解析快捷方式并提取目标文件的图标
/// 这是最简单可靠的方法
#[cfg(target_os = "windows")]
fn extract_lnk_icon(lnk_path: &str) -> Result<DynamicImage, String> {
  // ExtractIconExW 会自动处理 LNK 文件，解析快捷方式并提取目标文件的图标
  // 这是 Windows API 的标准行为，无需手动解析快捷方式
