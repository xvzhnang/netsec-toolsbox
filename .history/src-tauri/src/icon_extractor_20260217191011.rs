...
fn ensure_icons_dir_sync() -> Result<std::path::PathBuf, String> {
    let dir = get_icons_dir();
    fs::create_dir_all(&dir).map_err(|e| format!("创建图标目录失败: {}", e))?;
    Ok(dir)
}
...
mod tests {
    ...
    #[test]
    fn extract_exe_icon_works_for_system_binary() {
        let windir = std::env::var("WINDIR").unwrap_or_else(|_| "C:\\Windows".to_string());
        let candidate = format!("{}\\System32\\notepad.exe", windir);
        let img = extract_exe_icon(&candidate).expect("should extract icon from system exe");
...
fn process_icon_to_base64(img: DynamicImage, size: u32) -> Result<String, String> {
    ...
    let mut png_data = Vec::new();
    {
        ...
        encoder
            .write_image(&rgba, size, size, image::ColorType::Rgba8.into())
...
"#;
    ...
    if output.status.success() {
        ...
        image::load_from_memory(&image_bytes).map_err(|e| format!("加载图片失败: {}", e))
    } else {
...
fn extract_lnk_icon(lnk_path: &str) -> Result<DynamicImage, String> {
    // 移除 Windows 长路径前缀 (\\?\)，因为 PowerShell 可能不支持
    let clean_path = lnk_path.strip_prefix("\\\\?\\").unwrap_or(lnk_path);
...
fn extract_html_icon(html_path: &str) -> Result<DynamicImage, String> {
    // 读取 HTML 文件内容
    let content =
        fs::read_to_string(html_path).map_err(|e| format!("读取 HTML 文件失败: {}", e))?;

    // 查找 favicon 链接
    let favicon_re = Regex::new(
        r#"(?i)<link[^>]+rel=["'](?:icon|shortcut\s+icon)["'][^>]*href=["']([^"']+)["']"#,
    )
    ...
    if let Some(cap) = favicon_re.captures(&content) {
        if let Some(favicon_path) = cap.get(1) {
            ...
            // 如果是相对路径，转换为绝对路径
            let favicon_abs_path = if favicon_url.starts_with("http://")
                || favicon_url.starts_with("https://")
            {
...
pub fn extract_icon_from_file(params: ExtractIconParams) -> Result<String, String> {
    let file_path = params.file_path.trim().trim_matches('`').trim().to_string();
    let tool_type = params.tool_type;

    let icons_dir = ensure_icons_dir_sync()?;

    if file_path.starts_with("http://") || file_path.starts_with("https://") {
        return Err("URL 类型请使用 fetch_favicon 命令".to_string());
    }
    ...
    let cache_path = icons_dir.join(format!("{}.png", cache_key));
    ...
    log::info!(
        "图标提取成功: file_path={}, base64_length={}",
        file_path_str,
        base64.len()
    );
    ...
    if actual_width != 160 || actual_height != 160 {
        ...
        let mut png_data = Vec::new();
        {
            ...
            let encoder = PngEncoder::new(&mut png_data);
            encoder
                .write_image(&rgba, 160, 160, image::ColorType::Rgba8.into())
                .map_err(|e| format!("PNG 编码失败: {}", e))?;
        }
        fs::write(&cache_path, &png_data).map_err(|e| format!("保存图标缓存失败: {}", e))?;
    } else {
        // 尺寸正确，直接保存
        let mut png_data = Vec::new();
        {
...
pub fn save_icon_to_cache(icon_data: String) -> Result<String, String> {
    ...
    // 验证是否为有效的图片
    let _img =
        image::load_from_memory(&image_bytes).map_err(|e| format!("无效的图片数据: {}", e))?;
    ...
    // 保存文件
    fs::write(&cache_path, &image_bytes).map_err(|e| format!("保存图标缓存失败: {}", e))?;
...
pub fn fetch_favicon(params: crate::types::FetchFaviconParams) -> Result<String, String> {
    ...
    // 解析 URL
    let url = url::Url::parse(&url_str).map_err(|e| format!("无效的 URL: {}", e))?;
