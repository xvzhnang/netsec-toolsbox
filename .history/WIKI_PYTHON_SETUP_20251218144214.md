...
1. **智能启动机制**：
   - 当调用 `start_wiki_server` 命令时，系统会自动检测 Python 环境
   - 如果 Python 可用且依赖完整，使用 Python 服务器
   - 如果 Python 不可用或缺少依赖，自动使用 Rust HTTP 服务器
   - 服务器监听 `127.0.0.1:8777` 端口
...
确保 Python 已正确安装并添加到 PATH。Wiki 服务器会自动检测并使用系统 Python，无需 conda。
...
**注意**：如果 Python 缺少依赖，Wiki 服务器会自动回退到 Rust 服务器，无需手动处理。
...
- **自动选择**：系统会自动选择 Python 或 Rust 服务器，无需手动配置
- **无需 conda**：可以使用任何 Python 环境，不限于 conda
- **自动回退**：如果 Python 不可用，自动使用 Rust 服务器
- **功能一致**：两种服务器功能完全一致
- **主题文件**：应放在 `wiki/themes/` 目录下
- **文档文件**：应放在 `wiki/docs/` 目录下
