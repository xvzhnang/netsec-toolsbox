...
class VLLMProvider(BaseProvider):
    """vLLM 本地模型提供商（兼容 OpenAI API 格式）"""
    
    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        self.api_url = config.get('api_url', 'http://localhost:8000/v1')
        self.model = config.get('model', '')
        self.api_key = config.get('api_key', 'EMPTY')  # vLLM 通常不需要真实的 API key
        self.client = None
        ...
        if HAS_OPENAI_LIB:
            try:
                ...
                if 'SIGALRM' in str(e):
                    print(f"⚠️ vLLM 客户端初始化失败（Windows 不支持 SIGALRM）: {e}", flush=True)
                else:
                    print(f"⚠️ vLLM 客户端初始化失败: {e}", flush=True)
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=1) as executor:
                ...
                try:
                    ...
                    return f"❌ vLLM API 请求超时（{request_timeout}秒）"
            
        except Exception as e:
            return f"❌ vLLM API 调用失败: {str(e)}"
