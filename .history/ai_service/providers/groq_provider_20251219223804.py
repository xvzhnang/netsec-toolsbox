...
class GroqProvider(BaseProvider):
    """Groq 快速推理服务提供商（兼容 OpenAI API 格式）"""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.api_key and HAS_OPENAI_LIB:
            try:
                ...
                if 'SIGALRM' in str(e):
                    print(f"⚠️ Groq 客户端初始化失败（Windows 不支持 SIGALRM）: {e}", flush=True)
                else:
                    print(f"⚠️ Groq 客户端初始化失败: {e}", flush=True)
    ...
    def chat(self, messages: List[Dict[str, str]], timeout: Optional[int] = None) -> str:
        ...
        if not self.is_available():
            return "❌ Groq 未配置或不可用，请检查 API 密钥"
        ...
        try:
            ...
            request_timeout = timeout or self.config.get('timeout', 30)  # Groq 通常很快
            ...
            with ThreadPoolExecutor(max_workers=1) as executor:
                ...
                try:
                    ...
                    return f"❌ Groq API 请求超时（{request_timeout}秒）"
