...
class TogetherProvider(BaseProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.api_key = config.get('api_key') or os.environ.get('TOGETHER_API_KEY')
        self.model = config.get('model', 'meta-llama/Llama-3-8b-chat-hf')
        ...
        if self.api_key and HAS_OPENAI_LIB:
            try:
                ...
                if 'SIGALRM' in str(e):
                    print(f"⚠️ Together AI 客户端初始化失败（Windows 不支持 SIGALRM）: {e}", flush=True)
                else:
                    print(f"⚠️ Together AI 客户端初始化失败: {e}", flush=True)
    ...
    def chat(self, messages: List[Dict[str, str]], timeout: Optional[int] = None) -> str:
        ...
        if not self.is_available():
            return "❌ Together AI 未配置或不可用，请检查 API 密钥"
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=1) as executor:
                ...
                try:
                    ...
                    return f"❌ Together AI API 请求超时（{request_timeout}秒）"
            
        except Exception as e:
            return f"❌ Together AI API 调用失败: {str(e)}"
