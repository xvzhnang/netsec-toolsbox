...
class LMStudioProvider(BaseProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if HAS_OPENAI_LIB:
            try:
                ...
                if 'SIGALRM' in str(e):
                    print(f"⚠️ LM Studio 客户端初始化失败（Windows 不支持 SIGALRM）: {e}", flush=True)
                else:
                    print(f"⚠️ LM Studio 客户端初始化失败: {e}", flush=True)
    ...
    def get_available_models(self) -> Optional[List[str]]:
        ...
        try:
            ...
            print(f"⚠️ 获取 LM Studio 模型列表失败: {e}", flush=True)
        return None
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=1) as executor:
                ...
                try:
                    ...
                    return f"❌ LM Studio API 请求超时（{request_timeout}秒）"
