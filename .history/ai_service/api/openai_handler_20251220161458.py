...
class AIRequestHandler(BaseHTTPRequestHandler):
    ...
    def handle(self):
        """重写 handle 方法，添加调试信息和异常保护"""
        try:
            print(f"[HANDLER] ===== 收到新请求 =====", file=sys.stderr, flush=True)
            # 注意：self.command 和 self.path 是在 BaseHTTPRequestHandler.handle() 内部设置的
            # 所以先调用 super().handle() 来初始化这些属性
            print(f"[HANDLER] 客户端地址: {getattr(self, 'client_address', 'unknown')}", file=sys.stderr, flush=True)
            
            # 调用父类的 handle()，它会解析请求并设置 self.command, self.path 等属性
            super().handle()
            
            # 现在可以安全访问这些属性了
            print(f"[HANDLER] 请求方法: {getattr(self, 'command', 'unknown')}", file=sys.stderr, flush=True)
