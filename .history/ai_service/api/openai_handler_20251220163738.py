    ...
    def do_GET(self):
        ...
        try:
            ...
            if path == '/v1/models':
                ...
                print(f"[HANDLER] 处理 /health 健康检查请求", file=sys.stderr, flush=True)
                try:
                    self._send_json_response({"status": "ok"})
                    print(f"[HANDLER] /health 响应已发送", file=sys.stderr, flush=True)
                except Exception as e:
                    print(f"[HANDLER] [ERROR] /health 响应发送失败: {type(e).__name__}: {e}", file=sys.stderr, flush=True)
                    traceback.print_exc(file=sys.stderr)
                    try:
