    ...
    def do_GET(self):
        ...
        try:
            ...
            if path == '/v1/models':
                ...
                print(f"[HANDLER] [HEALTH] 开始处理 /health 健康检查请求", file=sys.stderr, flush=True)
                ...
                try:
                    ...
                    print(f"[HANDLER] [HEALTH] /health 响应已发送 (响应耗时: {response_elapsed:.2f}ms, 总耗时: {total_elapsed:.2f}ms)", file=sys.stderr, flush=True)
                except Exception as e:
            ...
            if loop:
                ...
                try:
                    ...
                    if loop.is_closed():
                        ...
                        # 清除线程的默认事件循环设置，避免影响后续请求
                        try:
                            asyncio.set_event_loop(None)
                            print(f"[REQUEST-{request_id}] [STEP-3.4] 已清除线程的默认事件循环设置", file=sys.stderr, flush=True)
                        except:
                            pass
                        return
                    
                    print(f"[REQUEST-{request_id}] [STEP-3.4] 关闭事件循环并清除线程设置", file=sys.stderr, flush=True)
                    # 先清除线程的默认事件循环设置，避免影响后续请求
                    try:
