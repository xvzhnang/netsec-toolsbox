...
class AIRequestHandler(BaseHTTPRequestHandler):
    ...
    def __init__(self, *args, router: Router, **kwargs):
        ...
        try:
            ...
            if AIRequestHandler._init_count_lock is None:
                import threading
                AIRequestHandler._init_count_lock = threading.Lock()
            
            with AIRequestHandler._init_count_lock:
                AIRequestHandler._init_count += 1
                if AIRequestHandler._init_count == 1:
                    print(f"[HANDLER-INIT] 初始化 AIRequestHandler (首次)", file=sys.stderr, flush=True)
                elif AIRequestHandler._init_count <= 5:
                    # 前5次初始化都打印，便于调试
                    print(f"[HANDLER-INIT] 初始化 AIRequestHandler (第 {AIRequestHandler._init_count} 次)", file=sys.stderr, flush=True)
                elif AIRequestHandler._init_count % 100 == 0:
                    # 之后每100次打印一次
                    print(f"[HANDLER-INIT] 已初始化 {AIRequestHandler._init_count} 次", file=sys.stderr, flush=True)
