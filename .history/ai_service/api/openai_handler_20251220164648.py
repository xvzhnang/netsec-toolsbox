...
class AIRequestHandler(BaseHTTPRequestHandler):
    ...
    def __init__(self, *args, router: Router, **kwargs):
        ...
        try:
            ...
            with AIRequestHandler._init_lock:
                ...
                should_log = (
                    count == 1 or  # 首次初始化
                    count <= 5 or  # 前5次初始化都打印，便于调试
                    count % 100 == 0  # 之后每100次打印一次
                )
            ...
            if should_log:
                if count == 1:
                    ...
                    print(f"[HANDLER-INIT] 初始化 AIRequestHandler (第 {count} 次)", file=sys.stderr, flush=True)
                else:
    ...
    def do_GET(self):
        ...
        try:
            # 获取请求头中的调试信息
            health_check_id = self.headers.get('X-Health-Check-Id', 'unknown')
            health_check_time = self.headers.get('X-Health-Check-Time', 'unknown')
            
            print(f"[HANDLER] ===== 收到 GET 请求 =====", file=sys.stderr, flush=True)
            print(f"[HANDLER] 客户端地址: {getattr(self, 'client_address', 'unknown')}", file=sys.stderr, flush=True)
            print(f"[HANDLER] 请求路径: {getattr(self, 'path', 'unknown')}", file=sys.stderr, flush=True)
            if health_check_id != 'unknown':
                print(f"[HANDLER] 健康检查 ID: {health_check_id}, 前端时间戳: {health_check_time}", file=sys.stderr, flush=True)
            ...
            if path == '/v1/models':
                ...
                print(f"[HANDLER] [HEALTH-{health_check_id}] 开始处理 /health 健康检查请求", file=sys.stderr, flush=True)
                try:
                    response_start = time.time()
                    self._send_json_response({"status": "ok"})
                    response_elapsed = (time.time() - response_start) * 1000
                    total_elapsed = (time.time() - request_start_time) * 1000
                    print(f"[HANDLER] [HEALTH-{health_check_id}] /health 响应已发送 (响应耗时: {response_elapsed:.2f}ms, 总耗时: {total_elapsed:.2f}ms)", file=sys.stderr, flush=True)
                except Exception as e:
                    elapsed = (time.time() - request_start_time) * 1000
                    print(f"[HANDLER] [HEALTH-{health_check_id}] [ERROR] /health 响应发送失败 (耗时: {elapsed:.2f}ms): {type(e).__name__}: {e}", file=sys.stderr, flush=True)
                    traceback.print_exc(file=sys.stderr)
                    try:
        ...
        try:
            ...
            if loop:
                try:
                    # 检查事件循环是否已关闭
                    if loop.is_closed():
                        print(f"[REQUEST-{request_id}] [STEP-3] 事件循环已关闭，跳过清理", file=sys.stderr, flush=True)
                        return
                    ...
                    try:
                        ...
                        print(f"[REQUEST-{request_id}] [STEP-3.1] 所有任务数: {len(all_tasks)}", file=sys.stderr, flush=True)
                        ...
                        for task in all_tasks:
                            ...
                            if "Task-" in task_name:
                                ...
                                if hasattr(task, '_is_background') and task._is_background:
                                    print(f"[REQUEST-{request_id}] [STEP-3.1] 保留后台任务: {task_name}", file=sys.stderr, flush=True)
                                    continue
                            temp_tasks.append(task)
                        
                        print(f"[REQUEST-{request_id}] [STEP-3.1] 临时任务数: {len(temp_tasks)}", file=sys.stderr, flush=True)
                        pending = temp_tasks
                    except RuntimeError as e:
                        if "Event loop is closed" in str(e):
                            print(f"[REQUEST-{request_id}] [STEP-3.1] 事件循环已关闭，跳过任务清理", file=sys.stderr, flush=True)
                            return
                        raise
                    ...
                    for task in pending:
                        try:
                            task_name = task.get_name() if hasattr(task, 'get_name') else str(task)
                            print(f"[REQUEST-{request_id}] [STEP-3.2] 取消临时任务: {task_name}", file=sys.stderr, flush=True)
                            task.cancel()
                    ...
                    if pending:
                        try:
                            print(f"[REQUEST-{request_id}] [STEP-3.3] 等待临时任务取消完成（{len(pending)} 个任务）", file=sys.stderr, flush=True)
                            ...
                            print(f"[REQUEST-{request_id}] [STEP-3.3] 临时任务取消完成", file=sys.stderr, flush=True)
                        except asyncio.TimeoutError:
                            ...
                            if "Event loop is closed" in str(e) or "This event loop is already running" in str(e):
                                print(f"[REQUEST-{request_id}] [STEP-3.3] 事件循环状态异常，跳过等待: {e}", file=sys.stderr, flush=True)
                            else:
                                print(f"[REQUEST-{request_id}] [STEP-3.3] 等待任务取消失败: {e}", file=sys.stderr, flush=True)
                                traceback.print_exc(file=sys.stderr)
                        except Exception as wait_error:
                            print(f"[REQUEST-{request_id}] [STEP-3.3] 等待任务取消失败: {wait_error}", file=sys.stderr, flush=True)
                            traceback.print_exc(file=sys.stderr)
                    else:
                        print(f"[REQUEST-{request_id}] [STEP-3.3] 没有临时任务需要取消", file=sys.stderr, flush=True)
                ...
                try:
                    # 再次检查事件循环是否已关闭
                    if loop.is_closed():
                        print(f"[REQUEST-{request_id}] [STEP-3.4] 事件循环已关闭，跳过关闭操作", file=sys.stderr, flush=True)
                        return
                    ...
                    if "Event loop is closed" in str(e):
                        print(f"[REQUEST-{request_id}] [STEP-3.4] 事件循环已关闭，无需再次关闭", file=sys.stderr, flush=True)
                    else:
                        print(f"[REQUEST-{request_id}] [STEP-3.4] 关闭事件循环失败: {e}", file=sys.stderr, flush=True)
                        traceback.print_exc(file=sys.stderr)
