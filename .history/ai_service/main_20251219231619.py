...
try:
    from providers.tgi_provider import TGIProvider
    HAS_TGI = True
except ImportError:
    HAS_TGI = False
    ...
    with _providers_lock:
        ...
        if HAS_OPENAI and config.get('openai', {}).get('api_key'):
            try:
                ...
                if 'SIGALRM' in str(e):
                    print(f"⚠️ Windows 不支持 SIGALRM，跳过 OpenAI provider 初始化", file=sys.stderr, flush=True)
                else:
        ...
        if HAS_DEEPSEEK and config.get('deepseek', {}).get('api_key'):
            try:
                ...
                if 'SIGALRM' in str(e):
                    print(f"⚠️ Windows 不支持 SIGALRM，跳过 DeepSeek provider 初始化", file=sys.stderr, flush=True)
                else:
        ...
        if HAS_MISTRAL and config.get('mistral', {}).get('api_key'):
            try:
                ...
                if 'SIGALRM' in str(e):
                    print(f"⚠️ Windows 不支持 SIGALRM，跳过 Mistral provider 初始化", file=sys.stderr, flush=True)
                else:
                    print(f"⚠️ 初始化 Mistral provider 失败: {e}", file=sys.stderr, flush=True)
            except Exception as e:
                print(f"⚠️ 初始化 Mistral provider 失败: {e}", file=sys.stderr, flush=True)
        
        if HAS_GROQ and config.get('groq', {}).get('api_key'):
            try:
                providers['groq'] = GroqProvider(config.get('groq', {}))
            except AttributeError as e:
                if 'SIGALRM' in str(e):
                    print(f"⚠️ Windows 不支持 SIGALRM，跳过 Groq provider 初始化", file=sys.stderr, flush=True)
                else:
                    print(f"⚠️ 初始化 Groq provider 失败: {e}", file=sys.stderr, flush=True)
            except Exception as e:
                print(f"⚠️ 初始化 Groq provider 失败: {e}", file=sys.stderr, flush=True)
        
        if HAS_TOGETHER and config.get('together', {}).get('api_key'):
            try:
                providers['together'] = TogetherProvider(config.get('together', {}))
            except AttributeError as e:
                if 'SIGALRM' in str(e):
                    print(f"⚠️ Windows 不支持 SIGALRM，跳过 Together AI provider 初始化", file=sys.stderr, flush=True)
                else:
                    print(f"⚠️ 初始化 Together AI provider 失败: {e}", file=sys.stderr, flush=True)
            except Exception as e:
                print(f"⚠️ 初始化 Together AI provider 失败: {e}", file=sys.stderr, flush=True)
        ...
        if HAS_LMSTUDIO and config.get('lmstudio', {}).get('api_url'):
            try:
                ...
                if 'SIGALRM' in str(e):
                    print(f"⚠️ Windows 不支持 SIGALRM，跳过 LM Studio provider 初始化", file=sys.stderr, flush=True)
                else:
        ...
        if HAS_VLLM and config.get('vllm', {}).get('api_url'):
            try:
                providers['vllm'] = VLLMProvider(config.get('vllm', {}))
            except AttributeError as e:
                if 'SIGALRM' in str(e):
                    print(f"⚠️ Windows 不支持 SIGALRM，跳过 vLLM provider 初始化", file=sys.stderr, flush=True)
                else:
                    print(f"⚠️ 初始化 vLLM provider 失败: {e}", file=sys.stderr, flush=True)
            except Exception as e:
                print(f"⚠️ 初始化 vLLM provider 失败: {e}", file=sys.stderr, flush=True)
        
        if HAS_LOCALAI and config.get('localai', {}).get('api_url'):
            try:
                providers['localai'] = LocalAIProvider(config.get('localai', {}))
            except AttributeError as e:
                if 'SIGALRM' in str(e):
                    print(f"⚠️ Windows 不支持 SIGALRM，跳过 LocalAI provider 初始化", file=sys.stderr, flush=True)
                else:
                    print(f"⚠️ 初始化 LocalAI provider 失败: {e}", file=sys.stderr, flush=True)
            except Exception as e:
                print(f"⚠️ 初始化 LocalAI provider 失败: {e}", file=sys.stderr, flush=True)
    ...
    def _send_json_response(self, data: Dict[str, Any], status_code: int = 200):
        ...
        try:
            ...
            if 'signal' not in error_msg.lower() and 'SIGALRM' not in error_msg:
                print(f"⚠️ 发送响应失败 [{error_type}]: {error_msg}", file=sys.stderr, flush=True)
    ...
    def do_GET(self):
        ...
        try:
            ...
            if parsed_path.path == '/health':
                ...
                # 获取指定提供商的模型列表
                provider_name = parsed_path.path.replace('/models/', '')
                if provider_name in providers:
                    try:
                        ...
                        if 'SIGALRM' in str(e):
                            self._send_error(f'Provider "{provider_name}" 在 Windows 上不可用（SIGALRM 不支持）', 500)
                        else:
                            self._send_error(f'获取模型列表失败: {str(e)}', 500)
                    except Exception as e:
                        self._send_error(f'获取模型列表失败: {str(e)}', 500)
                else:
                    self._send_error(f'Provider "{provider_name}" not found', 404)
            else:
        ...
        try:
            ...
            if parsed_path.path == '/chat':
                try:
                    ...
                    # 注意：provider.chat 内部已经使用了 ThreadPoolExecutor，这里不需要再次包装
                    # 但如果请求层面有自定义超时，仍然使用外层超时保护
                    # 安全获取超时配置
                    if hasattr(provider, 'config') and isinstance(provider.config, dict):
                        request_timeout = timeout or provider.config.get('timeout', 120)
                    else:
                        request_timeout = timeout or 120
                    
                    # 使用 ThreadPoolExecutor 实现跨平台超时（双重保护）
                    with ThreadPoolExecutor(max_workers=1) as executor:
                        future = executor.submit(provider.chat, messages, timeout)
                        try:
                            response_text = future.result(timeout=request_timeout + 10)  # 额外 10 秒缓冲
                        except FutureTimeoutError:
                            raise TimeoutError(f'AI 请求超时（{request_timeout}秒）')
                    
                    # 检查响应是否包含错误信息
                    if isinstance(response_text, str) and response_text.startswith('❌'):
                        # Provider 返回了错误信息，将其作为错误响应返回
                        self._send_error(response_text, 500)
                    else:
                        # 正常响应
                        self._send_json_response({
                            'success': True,
                    ...
                    if 'SIGALRM' in str(e):
                        error_msg = f'Windows 平台不支持 SIGALRM，请检查 provider 实现: {str(e)}'
                    ...
                    error_type = type(e).__name__
                    # 打印完整的错误堆栈以便调试
                    print(f"❌ [do_POST] 处理 /chat 请求时发生错误 [{error_type}]: {error_msg}", file=sys.stderr, flush=True)
                    traceback.print_exc(file=sys.stderr)
                    # 检查是否是 SIGALRM 相关错误
                    if 'SIGALRM' in error_msg:
                        print(f"❌ Windows 平台 SIGALRM 错误: {error_msg}", file=sys.stderr, flush=True)
                        self._send_error(f'Windows 平台不支持 SIGALRM，请检查 provider 实现', 500)
                    else:
                        # 返回更详细的错误信息（仅包含错误类型和消息，不包含堆栈）
                        self._send_error(f'Internal error [{error_type}]: {error_msg}', 500)
