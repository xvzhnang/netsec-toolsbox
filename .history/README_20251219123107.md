...
- **Wiki 文档系统**：完整的 Markdown 文档管理，支持 GitHub 风格渲染、代码高亮（highlight.js）、数学公式（KaTeX）、流程图（Mermaid）、主题自定义、搜索和目录导航
...
- **Markdown 渲染**：marked.js（npm 包，打包到 bundle）
- **代码高亮**：highlight.js（本地加载，位于 `public/highlight.js-11.11.1/`）
- **数学公式**：KaTeX（本地加载，位于 `public/katex/`）
- **流程图**：Mermaid（npm 包，打包到 bundle）
- **主题样式**：Typora See Yue Dark（本地加载，位于 `public/typora-see-yue-theme_v1.4/`）
