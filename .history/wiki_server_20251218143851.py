...
# Markdown 扩展配置（支持 GitHub 风格 Markdown）
md_extensions = [
    'codehilite',
    'fenced_code',
    'tables',
...
def render_markdown(file_path: Path) -> Dict[str, Any]:
    """渲染 Markdown 文件为 HTML（支持 GitHub 风格和 pymdown-extensions）"""
    try:
    ...
    # 配置 Markdown（尝试使用 pymdown-extensions 如果可用）
    try:
        ...
        # 使用 pymdown-extensions 增强的扩展
        enhanced_extensions = md_extensions + [
        ...
        md = markdown.Markdown(
            extensions=enhanced_extensions,
            extension_configs={
        ...
        # 如果 pymdown-extensions 不可用，使用标准扩展
        md = markdown.Markdown(
...
<body>
  ...
  <script>
    ...
    function toggleDir(dirId) {{
      ...
      if (dir) {{
        const isVisible = dir.style.display !== 'none';
        dir.style.display = isVisible ? 'none' : 'block';
        const toggle = dir.previousElementSibling;
        if (toggle) {{
          toggle.textContent = isVisible ? '▶ ' + toggle.textContent.replace('▼ ', '').replace('▶ ', '') : '▼ ' + toggle.textContent.replace('▶ ', '').replace('▼ ', '');
        }}
      }}
    }}
    ...
    function generateTOC() {{
      ...
      const tocSection = document.querySelector('.wiki-toc-section');
      if (!tocSection || headings.length === 0) return;
      
      let tocHTML = '<nav class="wiki-toc"><ul>';
      let currentLevel = 0;
      ...
      tocHTML += '</ul>'.repeat(currentLevel) + '</ul></nav>';
      tocSection.innerHTML = '<h3>页面目录</h3>' + tocHTML;
    }}
  </script>
</body>
</html>"""
...
if __name__ == '__main__':
    ...
    print(f"Wiki 根目录: {wiki_dir}", file=sys.stderr)
    print(f"Wiki 文档目录: {docs_dir}", file=sys.stderr)
    print(f"Wiki 主题目录: {theme_dir}", file=sys.stderr)
