...
// katex 插件（数学公式）- 在最后加载
// 重要：禁用 markdown-it-katex 的自动渲染，只让它生成占位符
// 实际的 KaTeX 渲染由 WikiView.vue 中的 renderKaTeXFormulas 统一处理
// 这样可以避免重复渲染和嵌套问题
if (katex && typeof katex === 'function') {
  try {
    md.use(katex, {
      ...
      strict: false,
      // 禁用自动渲染，只生成占位符
      // 这样 markdown-it-katex 会生成 <span class="katex"> 或 <span class="katex-display"> 元素
      // 但不会调用 katex.render，由后续的 renderKaTeXFormulas 统一渲染
      renderMathML: false,
      // 不自动渲染，只生成占位符
      macros: {}
    })
    debugLog('✅ [markdown-it] katex 插件已加载（仅生成占位符，不自动渲染）')
  } catch (err) {
    debugError('❌ [markdown-it] katex 插件加载失败:', err)
    debugError('错误详情:', err.message, err.stack)
  }
} else {
  debugError('❌ [markdown-it] katex 插件未加载，数学公式可能无法渲染')
  debugError('katex 类型:', typeof katex, katex)
  if (katexModule) {
    debugError('katexModule 内容:', Object.keys(katexModule))
  }
}
