  ...
  highlight: function(code, lang) {
    ...
    if (normalizedLang && hljs.getLanguage(normalizedLang)) {
      try {
        ...
        console.warn('[markdown-it] highlight.js 高亮失败:', err, '语言:', normalizedLang)
    ...
    if (!lang) {
      try {
        ...
        console.warn('[markdown-it] highlight.js 自动检测失败:', err)
...
if (mermaid) {
  try {
    ...
    if (typeof mermaid === 'function') {
      ...
      for (const key in mermaid) {
        if (typeof mermaid[key] === 'function') {
          ...
          console.log(`[markdown-it] 找到 mermaid 插件函数: ${key}`)
          break
        }
      }
    }
    ...
    if (mermaidPlugin && typeof mermaidPlugin === 'function') {
      ...
      console.log('✅ [markdown-it] mermaid 插件已加载')
    } else {
      console.error('❌ [markdown-it] mermaid 插件格式不正确:', typeof mermaid, mermaid)
  ...
  console.error('❌ [markdown-it] mermaid 插件未加载，Mermaid 图表可能无法渲染')
  console.error('mermaidModule:', mermaidModule)
  console.error('mermaidModule 类型:', typeof mermaidModule)
  if (mermaidModule && typeof mermaidModule === 'object') {
    console.error('mermaidModule 键:', Object.keys(mermaidModule))
  }
}
...
// katex 插件（数学公式）- 在最后加载
// 配置选项：支持块级公式（$$...$$）和行内公式（$...$）
// 重要：katex 插件必须在其他插件之后加载，避免与其他插件冲突
if (katex && typeof katex === 'function') {
  try {
    ...
    console.log('✅ [markdown-it] katex 插件已加载，支持块级和行内公式')
  } catch (err) {
    console.error('❌ [markdown-it] katex 插件加载失败:', err)
    console.error('错误详情:', err.message, err.stack)
  }
} else {
  console.error('❌ [markdown-it] katex 插件未加载，数学公式可能无法渲染')
  console.error('katex 类型:', typeof katex, katex)
  if (katexModule) {
    console.error('katexModule 内容:', Object.keys(katexModule))
  }
}
