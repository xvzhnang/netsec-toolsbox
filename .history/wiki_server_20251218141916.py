...
def api_search():
    ...
    wiki_dir = get_wiki_dir()
    results = search_wiki_files(query, wiki_dir)
...
def generate_toc_html(toc_items: List[Dict[str, Any]]) -> str:
    """生成目录 HTML"""
    if not toc_items:
        return '<nav class="wiki-toc"><p>暂无目录</p></nav>'
    # 简化实现，实际可以从 HTML 中提取
    return '<nav class="wiki-toc"><p>目录</p></nav>'

def generate_file_tree_html(files: List[Dict[str, Any]], level: int = 0) -> str:
    """生成文件树 HTML"""
    html = '<nav class="wiki-file-tree"><ul>'
    for file in files:
        if file['is_dir']:
            html += f'<li class="wiki-tree-dir" style="padding-left: {level * 16}px;"><span>📁 {file["name"]}</span>'
            if file.get('children'):
                html += generate_file_tree_html(file['children'], level + 1)
            html += '</li>'
        else:
            html += f'<li class="wiki-tree-file" style="padding-left: {level * 16}px;"><a href="/file/{file["path"]}">{file["title"]}</a></li>'
    html += '</ul></nav>'
    return html
...
if __name__ == '__main__':
    ...
    print(f"Wiki 服务器启动在 http://{HOST}:{PORT}")
