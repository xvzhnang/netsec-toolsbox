  ...
  return new Promise((resolve, reject) => {
    ...
    const tryLoadScript = () => {
      if (currentPathIndex >= localPaths.length) {
        debugError('❌ [markdown-it] 无法从本地加载，请确保文件存在于 public/markdown-it/ 目录')
        debugError('尝试的路径:', localPaths)
      ...
      script.onload = () => {
        ...
        setTimeout(() => {
          ...
          if (typeof win.markdownit !== 'undefined') {
            ...
            debugLog('✅ [markdown-it] 加载成功:', localPaths[currentPathIndex])
            resolve(mdInstance)
          } else {
            // 调试：检查所有可能的全局变量
            debugError('❌ [markdown-it] 脚本已加载，但未找到全局变量 markdownit')
            debugError('可用的全局变量:', {
