...
export type TauriListen = (event: string, handler: (event: any) => void) => Promise<TauriUnlisten>
...
export function getTauriInvoke(): TauriInvoke | null {
  try {
    ...
    logError('获取 Tauri API 时出错:', error)
    return null
  }
}

export function getTauriListen(): TauriListen | null {
  try {
    const win = window as any
    const tauri = win.__TAURI__
    const internals = win.__TAURI_INTERNALS__

    const candidates: Array<{ fn: any; owner: any }> = [
      { fn: tauri?.event?.listen, owner: tauri?.event },
      { fn: tauri?.tauri?.event?.listen, owner: tauri?.tauri?.event },
      { fn: tauri?.core?.event?.listen, owner: tauri?.core?.event },
      { fn: internals?.event?.listen, owner: internals?.event },
      { fn: internals?.tauri?.event?.listen, owner: internals?.tauri?.event },
    ]

    for (const c of candidates) {
      if (typeof c.fn === 'function') {
        return c.fn.bind(c.owner) as TauriListen
      }
    }

    return null
  } catch (error) {
    logError('获取 Tauri listen 时出错:', error)
    return null
  }
}
...
export async function openUrlInBrowser(url: string): Promise<void> {
  try {
    ...
    if (invoker) {
      // 使用 open_url_in_browser 命令（直接传递 url 参数，不需要 params 包装）
      try {
        await invoker('open_url_in_browser', { url })
        return
      } catch (err) {
        // 如果命令失败，降级到 window.open
        warn('通过后端打开浏览器失败，降级到 window.open:', err)
        // 继续执行降级逻辑
      }
    }
    
    // 降级到 window.open（如果 Tauri API 不可用或命令失败）
    const opened = window.open(url, '_blank', 'noopener,noreferrer')
    if (!opened) {
      // 如果 window.open 也被阻止，尝试使用 location.href（作为最后的降级方案）
      // 但这会导航当前页面，所以只在没有其他选择时使用
      warn('window.open 被阻止，无法打开 URL:', url)
      throw new Error('浏览器阻止了弹窗，请允许弹窗后重试')
    }
  } catch (err) {
    // 如果所有方法都失败，抛出错误
    const errorMessage = err instanceof Error ? err.message : String(err)
    logError('打开浏览器失败:', errorMessage)
    throw err
  }
}
