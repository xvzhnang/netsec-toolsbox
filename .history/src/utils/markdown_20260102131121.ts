...
const initMarkdownIt = async (): Promise<any> => {
  ...
  mdRenderer = new MarkdownIt({
    ...
    highlight: function (str: string, lang: string) {
      // Mermaid 特殊处理：保留原始内容，但转义 HTML 以防止浏览器解析
      // 这样 renderMermaidCharts 可以通过 textContent 获取到原始代码（含 < 等符号）
      if (lang && lang.toLowerCase() === 'mermaid') {
        return '<div class="mermaid">' + mdRenderer.utils.escapeHtml(str) + '</div>';
      }
      ...
      // 默认处理（转义 HTML）
      // 注意：这里不要添加 hljs 类，否则后续的 applyCodeHighlighting 会跳过它
      return '<pre><code class="language-' + lang + '">' + mdRenderer.utils.escapeHtml(str) + '</code></pre>';
    }
  })
