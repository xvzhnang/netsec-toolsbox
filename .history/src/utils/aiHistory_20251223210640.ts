...
export async function setCurrentSession(sessionId: string): Promise<void> {
  try {
    const invoker = getTauriInvoke()
    if (invoker) {
      await invoker('ai_history_set_current_session', { sessionId })
      return
    }

    const content = await readConfigFile(HISTORY_FILE)
    const historyFile: { sessions: ChatSession[]; currentSessionId?: string } =
      content && content.trim() ? JSON.parse(content) : { sessions: [] }
    historyFile.currentSessionId = sessionId
    await saveChatHistoryFile(historyFile)
  } catch (error) {
    logError('设置当前会话失败:', error)
  }
}
