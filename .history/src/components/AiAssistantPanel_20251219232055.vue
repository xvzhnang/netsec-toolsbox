...
import { sendAIChat, checkAIServiceHealth, getAvailableModels, waitForAIService, type AIMessage } from '../utils/aiService'
...
const currentModel = ref('gpt-3.5-turbo')  // 使用模型 ID 而不是 provider
...
const send = async () => {
  ...
  // 保存用户消息到历史记录
  await addMessageToHistory(userMsg as ChatMessage, currentModel.value)
  ...
  try {
    ...
    // 调用 AI 服务（使用模型 ID）
    const response = await sendAIChat(currentModel.value, aiMessages, {
      wikiContext,
      timeout: 120  // 2 分钟超时
    })
    ...
    if (response.success && response.response) {
      ...
      if (index !== -1) {
        ...
        if (msg) {
          ...
          // 更新历史记录中的消息
          await updateMessageInHistory(loadingMsg.id, response.response)
        }
      }
    } else {
      // 显示错误消息
      ...
      if (index !== -1) {
        ...
        if (msg) {
          ...
          msg.text = errorText
          // 更新历史记录中的错误消息
          await updateMessageInHistory(loadingMsg.id, errorText)
        }
      }
    }
  } catch (error) {
    ...
    if (index !== -1) {
      const msg = messages.value[index]
      if (msg) {
        const errorText = `❌ 请求失败: ${error instanceof Error ? error.message : String(error)}`
        msg.text = errorText
        // 更新历史记录中的错误消息
        await updateMessageInHistory(loadingMsg.id, errorText)
      }
  ...
  try {
    ...
    if (history.messages && history.messages.length > 0) {
      ...
      if (historyMessages.length > 0) {
        ...
        // 恢复提供商（如果有）
        if (history.provider && availableModels.value.includes(history.provider)) {
          currentModel.value = history.provider  // 历史记录中的 provider 字段实际是 model ID
        }
  ...
  saveTimer = setTimeout(async () => {
    try {
      ...
      if (messagesToSave.length > 0) {
        await saveMessagesToHistory(messagesToSave as ChatMessage[], currentModel.value)
      }
    } catch (error) {
      ...
      <div class="input-container" :class="{ 'focused': isInputFocused, 'disabled': !isServiceAvailable }">
        ...
        <div class="input-actions-row">
          <button 
            ...
            :title="`当前模型: ${getModelDisplayName(currentModel)}`"
