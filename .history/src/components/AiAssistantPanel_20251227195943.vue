...
import { getTauriListen, isTauriEnvironment, waitForTauriAPI } from '../utils/tauri'
...
const checkService = async (delayMs: number = 0) => {
  ...
  try {
    ...
    if (!state) {
      ...
      // 关键优化：如果服务被用户手动停止（stopped 状态），不进行健康检查，直接返回
      debug('[服务状态检测] 服务已停止（用户手动停止），跳过健康检查')
      return
    ...
    if (isHealthy) {
      ...
      try {
        ...
        if (models.length > 0) {
          ...
          debug('警告: 没有获取到任何模型')
          emptyModelsHits += 1
          if (availableModels.value.length === 0 || emptyModelsHits >= 3) {
...
onMounted(async () => {
  ...
  if (isTauriEnvironment() && !unlistenServiceEvent) {
    await waitForTauriAPI(5000).catch(() => false)

    const listenFn = getTauriListen()
    if (listenFn) {
      try {
        unlistenServiceEvent = await listenFn('service_event', (event: any) => {
      ...
      warn('[AiAssistantPanel] Tauri listen 不可用，service_event 将无法实时更新')
    }
  }
...
.session-select:focus-visible {
  box-shadow: 0 0 0 3px rgba(79, 193, 255, 0.18);
