...
import { openDocsify } from '../utils/docsifyLauncher'
...
const openWikiHome = async () => {
  try {
    ...
    logError('Failed to open Docsify:', error)
  }
}
...
const onOpenToolWiki = async (tool: ToolItem) => {
  ...
  try {
    ...
    if (tool.wikiUrl) {
      // Docsify 使用 hash 路由，例如 /#/tools/nmap
      // 如果 wikiUrl 是完整 URL，则直接使用
      if (tool.wikiUrl.startsWith('http')) {
        url = tool.wikiUrl
      } else {
        // 处理相对路径
        const path = tool.wikiUrl.startsWith('/') ? tool.wikiUrl.slice(1) : tool.wikiUrl
        // 如果没有扩展名且不以 / 结尾，Docsify 可能需要 .md 或者目录
        // 这里假设 wikiUrl 是相对于 docs 根目录的路径，例如 "tools/nmap"
        url = `${url}/#/${path}`
      }
    }
    ...
    logError('Failed to open Docsify for tool:', error)
  }
}
...
.wiki-panel-wrapper {
  /* flex: 0 0 450px; 由 inline style 控制 */
  max-width: 80%; /* 允许拉得更宽，原来的 50% 有点小 */
  min-width: 300px;
  display: flex;
  flex-direction: column;
  background: rgba(15, 23, 42, 0.95);
  ...
  height: 100%;
  overflow: hidden;
  transition: flex-basis 0.3s ease, transform 0.3s ease; /* 明确指定 transition 属性 */
  border-radius: 12px 0 0 12px;
  position: relative;
}

.wiki-panel-wrapper.is-resizing {
