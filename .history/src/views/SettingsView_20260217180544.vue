...
const aiServiceDetails = ref<any>(null)
...
const checkStatus = async () => {
  ...
  try {
    ...
    // 获取详细信息
    try {
      const details = await getAIServiceDetails()
      if (details) {
        aiServiceDetails.value = details
      }
    } catch (e) {
      debug('获取服务详细信息失败:', e)
    }
...
const handleRestartWorker = async (workerId: number) => {
  if (!confirm(`确定要重启 Worker-${workerId} 吗？`)) return
  try {
    info(`正在重启 Worker-${workerId}...`)
    await restartAIWorker(workerId)
    await new Promise(resolve => setTimeout(resolve, 1000))
    await checkStatus()
    info(`Worker-${workerId} 已重启`)
  } catch (error) {
    logError(`重启 Worker-${workerId} 失败:`, error)
    alert(`重启失败: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
<template>
  <AppLayout title="设置中心" subtitle="管理全局配置和 AI Gateway 服务。" :show-back="true">
    <template #sidebar>
    ...
    <div class="settings-content-inner">
      ...
      <div id="ai-gateway" class="config-panel" :class="{ hidden: activeTab !== 'service' }">
          ...
          <div class="config-section">
            ...
            <div v-if="aiServiceStatus === 'running' && aiServiceDetails?.metadata?.workers" class="section-item">
              <div class="section-label">
                <span class="label-text">任务管理</span>
                <span class="label-desc">管理 AI Gateway 的工作进程 (PID/端口)</span>
              </div>
              <div class="section-value">
                <div class="workers-list">
                  <div v-for="worker in aiServiceDetails.metadata.workers" :key="worker.id" class="worker-item">
                    <div class="worker-info">
                      <div class="worker-header">
                        <span class="worker-id">Worker-{{ worker.id }}</span>
                        <span class="worker-status-badge" :class="worker.is_healthy ? 'healthy' : 'unhealthy'">
                          {{ worker.is_healthy ? '健康' : '异常' }}
                        </span>
                      </div>
                      <div class="worker-meta">
                        <span class="meta-item" title="进程 ID">
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="8" rx="2" ry="2"></rect><rect x="2" y="14" width="20" height="8" rx="2" ry="2"></rect><line x1="6" y1="6" x2="6.01" y2="6"></line><line x1="6" y1="18" x2="6.01" y2="18"></line></svg>
                          PID: <span class="mono">{{ worker.pid || '无' }}</span>
                        </span>
                        <span class="meta-item" title="监听端口">
                          ...
                          Port: <span class="mono">{{ worker.port }}</span>
                        </span>
                        <span class="meta-item status" title="详细状态">
                          状态: {{ worker.status }}
                        </span>
                      </div>
                    </div>
                    <button class="action-btn restart-btn-small" @click="handleRestartWorker(worker.id)" title="重启进程">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
/* Removed settings-content style */
...
.restart-btn-small:hover:not(:disabled) {
