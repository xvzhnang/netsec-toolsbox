  ...
  try {
    ...
    if (wikiUrl) {
      ...
      if (wikiUrl.startsWith('http://') || wikiUrl.startsWith('https://')) {
        ...
        filePath = wikiUrl.trim()
      }
    }
    
    // 如果没有提供文件路径，尝试根据工具 ID 或名称自动查找
    if (!filePath && (toolId || toolName)) {
      try {
      ...
      <div class="modal-form">
        ...
        <label class="field">
          ...
          <input
            ...
            class="field-input"
            placeholder="例如：tools/tool-name.md 或留空自动查找"
          />
          <span class="field-hint">输入 Wiki 文件的相对路径（如 tools/tool-name.md），或留空让系统根据工具名称自动查找</span>
        </label>
        <div class="modal-form-actions">
