...
const searchItems = computed<GlobalSearchItem[]>(() => {
      ...
      categoriesRef.value
        ...
        .forEach((c) => {
          items.push({
            ...
            iconEmoji: getCategoryIcon(c.icon),
            originalData: c
          })
...
// 使用 useSearch 组合式函数
const { filteredResults } = useSearch(searchItems, query)

const goToSettings = () => {
  router.push({ name: 'settings' })
}

const onResultClick = async (item: GlobalSearchItem) => {
  ...
  } else if (item.type === 'tool' && item.originalData) {
    // 工具类型直接打开，不跳转
    await openTool(item.originalData as ToolItem)
  }
  query.value = ''
    ...
    <div class="dashboard-content">
      ...
      <div v-if="isAiOpen" class="content-row ai-open" @contextmenu="showBlankMenu" :class="{ 'search-overlay-active': filteredResults.length }">
        <SearchOverlay
          ...
          @close="query = ''; selectedSearchIndex = -1"
        />
