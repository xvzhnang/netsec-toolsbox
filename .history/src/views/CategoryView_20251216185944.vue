...
const category = computed(() => {
  ...
  // 确保数据已同步到categoriesData
  const existing = categoriesData.value.find((c) => c.id === cat.id)
  if (!existing) {
    categoriesData.value.push(cat)
  }
  // 返回categoriesData中的引用，以便直接修改
      ...
      <div class="content-row" @contextmenu="showBlankMenu">
        ...
        <section class="tools-area">
          <div v-if="!currentSub && subCategories.length === 0" class="empty-state">
            ...
            <h3>暂无子分类</h3>
            <p>点击左侧"添加子分类"按钮开始创建子分类</p>
          </div>
          <template v-else>
            <div class="tools-header">
              <div>
                <h2>{{ currentSub?.name || '请选择子分类' }}</h2>
                <p>{{ currentSub?.description || '选择一个子分类以查看工具' }}</p>
              </div>
              <div v-if="currentSub" class="tools-header-actions">
          ...
          >
            ...
            <div v-if="currentSub && filteredTools.length === 0 && !searchQuery" class="empty-state">
              <div class="empty-icon">🛠️</div>
              <h3>暂无工具</h3>
              <p>点击"新增工具"按钮添加工具到此子分类</p>
            </div>
            <div v-else-if="currentSub" class="tools-grid">
              <div
                v-for="tool in filteredTools"
                :key="tool.id"
                class="tool-card"
