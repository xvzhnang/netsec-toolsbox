...
// 从路由查询参数或 props 获取值
const filePath = ref<string | undefined>(props.filePath || (route.query.filePath as string | undefined))
const toolId = ref<string | undefined>(props.toolId || (route.query.toolId as string | undefined))
const toolName = ref<string | undefined>(props.toolName || (route.query.toolName as string | undefined))
...
watch(() => route.query, (newQuery) => {
  filePath.value = (newQuery.filePath as string | undefined) || props.filePath
  toolId.value = (newQuery.toolId as string | undefined) || props.toolId
  toolName.value = (newQuery.toolName as string | undefined) || props.toolName
  // 重新加载 Wiki 内容
  if (filePath.value || toolId.value) {
    loadWikiContent()
  }
}, { deep: true })
...
const loadFileTree = async () => {
  try {
    ...
    const files = await invoker<WikiFileInfo[]>('get_wiki_files')
...
const loadThemes = async () => {
  try {
    ...
    const themeList = await invoker<string[]>('get_wiki_themes')
...
const changeTheme = async () => {
  try {
    ...
    // 重新加载当前文件以应用新主题
    if (props.filePath) {
      await loadWikiFile(props.filePath)
    }
  } catch (err) {
    logError('切换主题失败:', err)
  }
...
const performSearch = async () => {
  ...
  try {
    ...
    const results = await invoker<Array<{ file_path: string; title: string }>>('search_wiki', {
...
const loadWikiContent = async () => {
  ...
  try {
    // 确定要加载的文件
    if (filePath.value) {
      await loadWikiFile(filePath.value)
