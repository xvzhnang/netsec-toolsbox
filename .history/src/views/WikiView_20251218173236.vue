  ...
  <div class="wiki-view" :class="{ 'wiki-view-modal': isModal }">
    ...
    <div v-else class="wiki-container">
      ...
      <main class="wiki-content">
        ...
        <div class="wiki-content-header">
          <div class="wiki-toolbar">
            ...
            <div class="accessibility-controls">
              ...
              <label class="high-contrast-toggle">
        ...
        <article class="markdown-body" :style="{ fontSize: fontSize + 'px' }" v-html="contentHtml"></article>
      </main>
...
const addLineNumbers = (container: HTMLElement) => {
  ...
  codeBlocks.forEach((codeElement) => {
    ...
    if (!preElement || preElement.classList.contains('has-line-numbers')) {
      return
    }
...
onMounted(async () => {
  ...
  // 加载用户设置
  const savedFontSize = localStorage.getItem('wiki-font-size')
  if (savedFontSize) {
    fontSize.value = parseInt(savedFontSize, 10)
  }
  
  const savedHighContrast = localStorage.getItem('wiki-high-contrast')
...
.code-theme-selector label {
  color: #656d76;
  font-weight: 500;
}

.code-theme-selector select {
  padding: 6px 12px;
  border: 1px solid #d0d7de;
  border-radius: 6px;
  background: #ffffff;
...
.code-theme-selector select:hover {
  border-color: #0969da;
}

.code-theme-selector select:focus {
  outline: none;
  border-color: #0969da;
