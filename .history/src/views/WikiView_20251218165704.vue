...
import { renderMarkdown, extractTitle, extractTOC, renderMermaidCharts } from '../utils/markdown'
...
const loadWikiFile = async (filePath: string) => {
  ...
  try {
    ...
    // Tauri 2.x 会自动将 camelCase 转换为 snake_case
    const markdownText = await invoker('read_wiki_file', {
      file_path: filePath
    }) as string
...
const loadThemeCSS = async (themeName: string) => {
  try {
    ...
    try {
      cssContent = await invoker('read_wiki_file', {
        file_path: themePath
      }) as string
    } catch (err) {
      // 如果主题文件不存在，使用默认样式
