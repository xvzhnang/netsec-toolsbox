    ...
    try {
      ...
      katexPlaceholders.forEach((el) => {
        try {
          ...
          // 关键检查：如果元素内部已经有渲染好的 KaTeX HTML（包含 .katex 类），说明已经渲染过
          // markdown-it-katex 渲染后会生成包含 .katex 类的元素
          // 如果元素已经有 .katex 子元素，或者元素本身有 class="katex"，说明已经渲染过
          const hasRenderedKatex = el.querySelector('.katex') || 
                                   el.classList.contains('katex') ||
                                   (el.children.length > 0 && Array.from(el.children).some(child => 
                                     child.classList.contains('katex') || 
                                     child.querySelector('.katex')
                                   ))
          
          if (hasRenderedKatex) {
            // 已经渲染过，标记为已处理并跳过（不再处理）
            processedElements.add(el)
            debug('跳过已渲染的 KaTeX 元素:', el)
            return
          }
          
          // 获取公式文本（从 data 属性或文本内容）
          let formula = (el as HTMLElement).dataset.formula || el.textContent || ''
          if (!formula.trim()) {
            return
          }
      ...
      const processBlockFormulas = () => {
        ...
        paragraphs.forEach((para) => {
          ...
          if (blockMatches.length > 0) {
            ...
            if (blockMatches.length === 1 && text.trim() === blockMatches[0].match.trim()) {
              ...
              for (let i = blockMatches.length - 1; i >= 0; i--) {
                const { match, formula, index } = blockMatches[i]
                try {
                  // 查找包含这个公式的文本节点
                  const walker = document.createTreeWalker(
                    para,
                  ...
                  if (textNode) {
                    ...
                    const beforeText = text.substring(0, localIndex)
                    const afterText = text.substring(localIndex + match.length)
