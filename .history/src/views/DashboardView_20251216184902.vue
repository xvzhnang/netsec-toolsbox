...
const categoriesRef = ref<CategoryConfig[]>([...initialCategories])
...
const categoryMenuItems = computed<MenuItem[]>(() => {
  ...
  return [
    ...
    },
    {
      ...
      icon: '🗑️',
      action: () => {
        if (confirm(`确定删除分类「${contextMenuTarget.value?.label || contextMenuTarget.value?.name}」？`)) {
          const idx = categoriesRef.value.findIndex((c) => c.id === contextMenuTarget.value?.id)
          if (idx !== -1) {
