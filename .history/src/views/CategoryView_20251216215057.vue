...
const selectLocalImage = async () => {
  try {
    ...
    // 显示处理中提示
    showConfirm('处理中', '正在裁剪和压缩图片...', () => {}, 'info')
...
// 监听图标来源变化，自动处理
watch(() => toolForm.value.iconSource, (newSource) => {
  if (newSource === 'executable' && toolForm.value.execPath) {
    // 自动尝试提取图标
      ...
      <div class="modal-form">
        ...
        <div v-if="toolForm.iconSource === 'url'" class="field">
          <label class="field">
            ...
            <span class="field-hint">远程图片 URL 地址</span>
          </label>
        </div>
