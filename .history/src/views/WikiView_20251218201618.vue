  ...
  <div class="wiki-view" :class="{ 'wiki-view-modal': isModal }">
    ...
    <div v-else class="wiki-container">
      ...
      <main class="wiki-content">
        ...
        <article class="markdown-body" v-html="contentHtml" :style="{ opacity: loading ? 0 : 1, transition: 'opacity 0.3s ease' }"></article>
      </main>
...
const loadWikiFile = async (filePath: string) => {
  ...
  if (contentElement) {
    (contentElement as HTMLElement).style.opacity = '0'
    (contentElement as HTMLElement).style.transition = 'opacity 0.2s ease-out'
  }
...
const applyCodeHighlighting = async (container: HTMLElement) => {
  ...
  codeBlocks.forEach((codeElement) => {
    ...
    if (preElement) {
      const langMatch = preElement.className.match(/language-(\w+)/)
      if (langMatch && langMatch[1] && !codeElement.className.includes(langMatch[1])) {
        codeElement.classList.add(`language-${langMatch[1]}`)
      }
    }
  })
}
...
const addCopyButtonsToCodeBlocks = (container: HTMLElement) => {
  ...
  // 获取所有代码块（包括终端样式的）
  const allCodeBlocks = container.querySelectorAll('pre code, .terminal-code-container code')
  
  allCodeBlocks.forEach((codeElement) => {
    // 对于终端样式的代码块，复制按钮已经在 addCopyButtonToTerminal 中添加
    const terminalWrapper = codeElement.closest('.terminal-wrapper')
    if (terminalWrapper) {
      // 检查是否已经有复制按钮
      if (!terminalWrapper.querySelector('.terminal-copy-button')) {
        addCopyButtonToTerminal(terminalWrapper as HTMLElement, codeElement as HTMLElement)
      }
      return
    }
