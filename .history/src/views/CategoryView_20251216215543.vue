...
import { selectImageFile, processImage, extractIconFromExecutable } from '../utils/imageProcessor'
...
const editTool = (tool: ToolItem) => {
  toolForm.value = {
    ...
    iconSource: tool.iconSource || 'url',
...
const saveTool = () => {
  ...
  const base: ToolItem = {
    ...
    iconSource: toolForm.value.iconUrl.trim() ? toolForm.value.iconSource : undefined,
...
const selectLocalImage = async () => {
  try {
    ...
    toolForm.value.iconSource = 'local'
  } catch (error) {
...
const extractExecutableIcon = async () => {
  if (!toolForm.value.execPath) {
    showConfirm('提示', '请先设置可执行路径', () => {}, 'warning')
    return
  }
  
  try {
    showConfirm('处理中', '正在从可执行文件提取图标...', () => {}, 'info')
    const iconData = await extractIconFromExecutable(toolForm.value.execPath)
    
    if (iconData) {
      toolForm.value.iconUrl = iconData
      toolForm.value.iconSource = 'executable'
      showConfirm('成功', '图标已提取', () => {}, 'info')
    } else {
      showConfirm('提示', '无法从可执行文件提取图标，此功能需要后端支持', () => {}, 'warning')
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : '提取图标失败'
    showConfirm('错误', `提取图标失败：${errorMessage}`, () => {}, 'warning')
  }
}
    ...
    >
      <div class="modal-form">
        <div class="tool-form-grid">
          ...
          <label class="field">
            <span class="field-label">图标来源</span>
            <select v-model="toolForm.iconSource" class="field-input">
              <option value="url">URL（远程图片）</option>
              <option value="local">本地图片</option>
              <option value="executable">应用自身图标</option>
            </select>
          </label>
        </div>
        <div v-if="toolForm.iconSource === 'url'" class="field">
          <label class="field">
            <span class="field-label">图标 URL</span>
            <input v-model="toolForm.iconUrl" class="field-input" placeholder="https://example.com/icon.png" type="url" />
            <span class="field-hint">远程图片 URL 地址（不支持本地文件路径，请使用"本地图片"模式）</span>
          </label>
          <div v-if="toolForm.iconUrl && !toolForm.iconUrl.startsWith('file://') && (toolForm.iconUrl.startsWith('http://') || toolForm.iconUrl.startsWith('https://') || toolForm.iconUrl.startsWith('data:'))" class="icon-preview">
            <img :src="toolForm.iconUrl" alt="图标预览" class="icon-preview-img" @error="handlePreviewError" />
          </div>
          <div v-else-if="toolForm.iconUrl && toolForm.iconUrl.startsWith('file://')" class="icon-preview">
            <div class="icon-preview-error">⚠️ 检测到本地文件路径，请切换到"本地图片"模式选择图片</div>
          </div>
        </div>
        <div v-else-if="toolForm.iconSource === 'local'" class="field">
          <label class="field">
            ...
            <span class="field-hint">选择本地图片后会自动裁剪为正方形并压缩为 160x160</span>
          </label>
          <div v-if="toolForm.iconUrl" class="icon-preview">
            <img :src="toolForm.iconUrl" alt="图标预览" class="icon-preview-img" @error="handlePreviewError" />
          </div>
        </div>
        <div v-else-if="toolForm.iconSource === 'executable'" class="field">
          <label class="field">
            ...
            <div class="field-with-button">
              <input
                v-model="toolForm.iconUrl"
                class="field-input"
                placeholder="从可执行文件提取图标"
                readonly
              />
              <button
                type="button"
                class="btn secondary file-select-btn"
                @click="extractExecutableIcon"
                :disabled="!toolForm.execPath"
              >
                提取图标
              </button>
            </div>
            <span class="field-hint">需要先设置可执行路径，然后从此文件提取图标（需要后端支持）</span>
          </label>
          <div v-if="toolForm.iconUrl" class="icon-preview">
            <img :src="toolForm.iconUrl" alt="图标预览" class="icon-preview-img" @error="handlePreviewError" />
          </div>
        ...
        <label class="field">
          ...
          <span class="field-hint">当未设置图标时使用</span>
        </label>
        <label class="field">
          <span class="field-label">描述</span>
          <textarea
