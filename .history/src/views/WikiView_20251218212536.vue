...
watch(() => props.filePath, (newFilePath) => {
  debug('WikiView props.filePath 变化:', { newFilePath, current: currentFilePath.value })
  if (newFilePath !== currentFilePath.value) {
    currentFilePath.value = newFilePath
    if (newFilePath) {
      debug('触发 loadWikiContent，因为 filePath 有值:', newFilePath)
      loadWikiContent()
    } else {
      debug('filePath 为空，不加载内容')
    }
  }
}, { immediate: true })
...
watch(() => route.query, (newQuery) => {
  ...
  debug('WikiView 路由查询参数变化:', { queryFilePath, queryToolId, queryToolName })
  
  currentFilePath.value = queryFilePath || props.filePath
  currentToolId.value = queryToolId || props.toolId
  currentToolName.value = queryToolName || props.toolName
...
const loadWikiFile = async (filePath: string) => {
  ...
  try {
    ...
    debug('Wiki 文件读取成功，长度:', markdownText.length)
...
const applyCodeHighlighting = async (container: HTMLElement) => {
  ...
  try {
    ...
    console.warn('无法加载 highlight.js:', err)
    return
  }
    ...
    if (lang && hljs.getLanguage(lang)) {
      try {
        ...
        try {
          ...
          const result = hljs.highlightAuto(code, ['javascript', 'python', 'bash', 'shell', 'json', 'yaml', 'xml', 'html', 'css', 'rust', 'go', 'java', 'cpp', 'c'])
          codeElement.innerHTML = result.value
          codeElement.classList.add('hljs')
        } catch (err) {
          // 忽略错误，保持原样
          console.debug('代码高亮失败:', err)
        }
      }
    } else {
      // 自动检测语言
...
/* 已移除终端样式相关 CSS，使用普通 GitHub 暗色样式 */
...
// 加载 highlight.js（从 public 目录或 CDN）
const loadHighlightJS = (): Promise<any> => {
  return new Promise((resolve, reject) => {
      ...
      script.onload = () => {
        ...
        const tryLoadCSS = () => {
          if (cssIndex >= cssPaths.length) {
            ...
            cdnLink.href = 'https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.11.1/styles/github-dark.min.css'
    ...
    const loadFromCDN = () => {
      ...
      cdnScript.src = 'https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.11.1/highlight.min.js'
      cdnScript.onload = () => {
        // 加载 CSS
        const link = document.createElement('link')
        link.rel = 'stylesheet'
      ...
      cdnScript.onerror = () => reject(new Error('无法加载 highlight.js'))
...
const loadWikiContent = async () => {
  ...
  try {
    ...
    if (currentFilePath.value) {
      ...
      // 加载首页
      debug('没有 filePath 和 toolId，加载首页 README.md')
