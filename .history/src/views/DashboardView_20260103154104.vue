...
// 打开 Docsify 文档
const openDocsify = async () => {
  try {
    // 启动本地 Docsify 服务器
    const port = await invoke<number>('start_docs_server')
    const url = `http://localhost:${port}`
    console.log('Docsify server started at:', url)
    
    // 使用系统默认浏览器打开
    await open(url)
  } catch (error) {
    console.error('Failed to open Docsify:', error)
    // 可以添加一个错误提示 Toast
  }
}
  ...
  <div class="page">
    <header class="page-header">
      ...
      <div class="header-actions">
        ...
        <button type="button" class="icon-button" @click="openDocsify">
          <span class="icon">📚</span>
