...
const changeCodeTheme = () => {
  ...
  const themeFile = themeMap[codeTheme.value] || 'github-dark.min.css'
...
const loadHighlightJS = (): Promise<any> => {
  return new Promise((resolve, reject) => {
    ...
    // 尝试从 public 目录加载（使用 CDN 构建版本路径）
    // 如果 public 目录下有构建好的文件，使用本地文件
    // 否则使用 CDN 作为备选
    const script = document.createElement('script')
    script.src = '/highlight.js-11.11.1/dist/highlight.min.js'
    script.onerror = () => {
      // 如果本地文件不存在，使用 CDN
    ...
    script.onload = () => {
      ...
      const link = document.createElement('link')
      link.rel = 'stylesheet'
      link.href = '/highlight.js-11.11.1/src/styles/github-dark.css'
      link.onerror = () => {
        // 如果本地 CSS 不存在，使用 CDN
