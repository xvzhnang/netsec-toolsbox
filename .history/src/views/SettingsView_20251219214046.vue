...
const loadProviderModels = async (provider: string) => {
  ...
  try {
    ...
    debug(`加载 ${provider} 模型列表失败:`, error)
  } finally {
    loadingModels.value[provider] = false
  }
}

// 监听配置变化，当 API 密钥或 URL 变化时重新加载模型列表
              ...
              <div class="config-form">
                ...
                <div class="form-group">
                  ...
                  <div style="display: flex; gap: 8px; align-items: center;">
                    <select
                      v-if="providerModels.together && providerModels.together.length > 0"
                      :value="aiConfig.together?.model || 'meta-llama/Llama-3-8b-chat-hf'"
                      @change="(e) => updateConfig('together', 'model', (e.target as HTMLSelectElement).value)"
                      style="flex: 1; padding: 8px; border-radius: 4px; border: 1px solid rgba(255, 255, 255, 0.1); background: rgba(0, 0, 0, 0.3); color: #fff;"
                    >
                      <option value="">选择模型...</option>
                      <option v-for="model in providerModels.together" :key="model" :value="model">
                        {{ model }}
                      </option>
                    </select>
                    <input
                    ...
                    <button
                      type="button"
                      @click="loadProviderModels('together')"
                      :disabled="loadingModels.together"
                      style="padding: 8px 12px; border-radius: 4px; border: 1px solid rgba(255, 255, 255, 0.1); background: rgba(255, 158, 200, 0.2); color: #FF9EC8; cursor: pointer; white-space: nowrap;"
                      title="刷新模型列表"
                    >
                      {{ loadingModels.together ? '加载中...' : '刷新' }}
                    </button>
                  </div>
                </div>
                <div class="form-group">
            ...
            <div class="provider-config">
              ...
              <div class="config-form">
                ...
                <div class="form-group">
                  ...
                  <div style="display: flex; gap: 8px; align-items: center;">
                    <select
                      v-if="providerModels.vllm && providerModels.vllm.length > 0"
                      :value="aiConfig.vllm?.model || ''"
                      @change="(e) => updateConfig('vllm', 'model', (e.target as HTMLSelectElement).value)"
                      style="flex: 1; padding: 8px; border-radius: 4px; border: 1px solid rgba(255, 255, 255, 0.1); background: rgba(0, 0, 0, 0.3); color: #fff;"
                    >
                      <option value="">选择模型...</option>
                      <option v-for="model in providerModels.vllm" :key="model" :value="model">
                        {{ model }}
                      </option>
                    </select>
                    <input
                    ...
                    <button
                      type="button"
                      @click="loadProviderModels('vllm')"
                      :disabled="loadingModels.vllm"
                      style="padding: 8px 12px; border-radius: 4px; border: 1px solid rgba(255, 255, 255, 0.1); background: rgba(255, 158, 200, 0.2); color: #FF9EC8; cursor: pointer; white-space: nowrap;"
                      title="刷新模型列表"
                    >
                      {{ loadingModels.vllm ? '加载中...' : '刷新' }}
                    </button>
                  </div>
                </div>
                <div class="form-group">
