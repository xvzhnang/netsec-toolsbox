...
import { getServiceStatus, startService, stopService, restartService as apiRestartService } from '../utils/serviceManager'
  ...
  async function fetchModels() {
    try {
      ...
      if (models.length > 0) {
        ...
        if (!models.includes(currentModel.value)) {
          currentModel.value = models[0] || 'gpt-3.5-turbo'
        }
      }
    } catch (e) {
      logError('获取模型列表失败:', e)
