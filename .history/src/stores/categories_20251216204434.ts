      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        return data.map((cat: any) => ({
          ...
          subCategories: (cat.sub_categories || cat.subCategories || []).map((sub: any) => ({
            ...
            tools: (sub.tools || []).map((tool: any) => ({
              id: tool.id,
              name: tool.name,
              description: tool.description,
              iconEmoji: tool.icon_emoji || tool.iconEmoji,
              wikiUrl: tool.wiki_url || tool.wikiUrl,
              toolType: tool.tool_type || tool.toolType,
              execPath: tool.exec_path || tool.execPath,
              args: tool.args,
              workingDir: tool.working_dir || tool.workingDir,
            })),
          })),
        }))
      }
