...
Promise.all([
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log('配置文件存在检查结果:', fileExists)
    
    if (!fileExists) {
      // eslint-disable-next-line no-console
      console.log('配置文件不存在，使用默认值创建配置文件', {
        categoriesCount: config.length,
        dataCount: data.length,
      })
      // 文件不存在，使用默认值创建
      // 确保使用当前加载的默认值保存（包含所有8个一级分类）
      await saveAllConfigToFile()
      
      // 再次检查文件是否创建成功
      const created = await configFileExists()
      if (created) {
        // eslint-disable-next-line no-console
        console.log('✅ 配置文件创建成功')
      } else {
        // eslint-disable-next-line no-console
        console.error('❌ 配置文件创建失败，请检查 Tauri API 是否可用')
      }
    } else {
      // eslint-disable-next-line no-console
