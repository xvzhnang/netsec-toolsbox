...
const loadCategoriesData = async (): Promise<CategoryPageData[]> => {
  ...
  try {
    ...
    if (fileContent && fileContent !== '{}') {
      const parsed = JSON.parse(fileContent)
      // 如果文件中有data字段，使用它；否则尝试从categories中提取
      const data = parsed.data || parsed.categories
              ...
              return {
                ...
                tools: ((subCategory.tools || []) as unknown[]).map((tool: unknown) => {
                  ...
                  return {
                    ...
                    toolType: (toolItem.tool_type || toolItem.toolType) ? String(toolItem.tool_type || toolItem.toolType) as ToolType : undefined,
                    ...
                    jarConfig: jarConfig ? {
                      jarPath: String((jarConfig as Record<string, unknown>).jar_path || (jarConfig as Record<string, unknown>).jarPath || ''),
                      javaPath: (jarConfig as Record<string, unknown>).java_path || (jarConfig as Record<string, unknown>).javaPath ? String((jarConfig as Record<string, unknown>).java_path || (jarConfig as Record<string, unknown>).javaPath) : undefined,
                      jvmArgs: Array.isArray((jarConfig as Record<string, unknown>).jvm_args || (jarConfig as Record<string, unknown>).jvmArgs) ? ((jarConfig as Record<string, unknown>).jvm_args || (jarConfig as Record<string, unknown>).jvmArgs as unknown[]).map((a: unknown) => String(a)) : undefined,
                      programArgs: Array.isArray((jarConfig as Record<string, unknown>).program_args || (jarConfig as Record<string, unknown>).programArgs) ? ((jarConfig as Record<string, unknown>).program_args || (jarConfig as Record<string, unknown>).programArgs as unknown[]).map((a: unknown) => String(a)) : undefined,
                    } : undefined,
                  }
                }),
              }
...
watch(
  ...
  async (newData) => {
    try {
      // 转换数据格式以便保存
      const dataToSave = newData.map((cat) => ({
        id: cat.id,
        name: cat.name,
        label: cat.label,
      ...
      const content = JSON.stringify({ data: dataToSave }, null, 2)
      await writeConfigFile(content)
    } catch (error) {
      // eslint-disable-next-line no-console
