...
const loadCategoriesConfig = (): CategoryConfig[] => {
  ...
  return loadFromStorage('categoriesConfig', defaultConfig)
}

// 首页分类配置（用于Dashboard）
export const categoriesConfig = ref<CategoryConfig[]>(loadCategoriesConfig())

// 监听分类配置变化，自动保存到本地存储
watch(
  categoriesConfig,
  (newConfig) => {
    saveToStorage('categoriesConfig', newConfig)
  },
  { deep: true },
)

// 从本地存储加载分类页面数据，如果没有则使用默认值
const loadCategoriesData = (): CategoryPageData[] => {
