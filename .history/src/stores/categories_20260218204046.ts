...
function transformToolItem(toolItem: Record<string, unknown>): ToolItem {
  ...
  return {
    ...
    pythonEnv: toolItem.python_env || toolItem.pythonEnv ? String(toolItem.python_env || toolItem.pythonEnv) : undefined,
    jarConfig: jarConfig ? (() => {
...
const CATEGORIES_SAVE_DEBOUNCE_MS = 500
...
const saveCategoriesConfig = async () => {
  try {
    ...
    if (lastSavedCategoriesContent !== null && content === lastSavedCategoriesContent) {
      debug('分类配置未变化，跳过写入 categories.json')
      return
    }
    ...
    lastSavedCategoriesContent = content
...
export function requestSaveCategoriesConfig(): void {
  if (!isDataInitialized) return

  categoriesSavePending = true
  if (categoriesSaveTimer) {
    clearTimeout(categoriesSaveTimer)
  }
  categoriesSaveTimer = setTimeout(() => {
    categoriesSaveTimer = null
    void flushSaveCategoriesConfig()
  }, CATEGORIES_SAVE_DEBOUNCE_MS)
}

export async function flushSaveCategoriesConfig(): Promise<void> {
  if (!isDataInitialized) return
  ...
  if (categoriesSaveRunner) {
    await categoriesSaveRunner
    return
  }

  categoriesSaveRunner = (async () => {
    while (categoriesSavePending) {
      categoriesSavePending = false
...
const TOOLS_SAVE_DEBOUNCE_MS = 500
...
export const saveToolsData = async () => {
  try {
    ...
    if (lastSavedToolsContent !== null && content === lastSavedToolsContent) {
      debug('工具数据未变化，跳过写入 tools.json')
      return
    }
...
export function requestSaveToolsData(): void {
  if (!isDataInitialized) return
  ...
  toolsSaveTimer = setTimeout(() => {
    toolsSaveTimer = null
    void flushSaveToolsData()
  }, TOOLS_SAVE_DEBOUNCE_MS)
}

export async function flushSaveToolsData(): Promise<void> {
  if (!isDataInitialized) return
  ...
  if (toolsSaveRunner) {
    await toolsSaveRunner
    return
  }

  toolsSaveRunner = (async () => {
    while (toolsSavePending) {
