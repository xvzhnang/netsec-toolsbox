...
onMounted(async () => {
  ...
  await runLoadStep('Initializing AI Module...', 40, async () => {
    // 异步加载 AI 模块，不阻塞 UI 渲染
    // 使用 import() 动态导入，Vite 会自动进行代码分割
    // 注意：这里不再 await，而是放入后台任务队列
    import('./components/AiAssistantPanel.vue').then(() => {
        console.log('AI Module preloaded in background');
    }).catch(e => {
        console.warn('Failed to preload AI module in background:', e);
    });
    
    // 给一点时间让进度条走一下，但不等待加载完成
