...
**对应 One API**: `relay/adaptor/tencent/`

**特性**:
- 需要 TC3-HMAC-SHA256 签名算法
- 支持流式响应
- API Key 格式：`app_id|secret_id|secret_key`

**请求头**:
- `Authorization`: TC3 签名（自动生成）
- `X-TC-Action`: 操作类型（ChatCompletions）
- `X-TC-Version`: API 版本（2023-09-01）
- `X-TC-Timestamp`: 时间戳
- `X-TC-Region`: 地域（如 ap-beijing）
...
**对应 One API**: `relay/adaptor/openai` + `moonshot/`

**特性**:
- OpenAI 兼容 API
- 标准 endpoint: `/v1/chat/completions`
...
**对应 One API**: `relay/adaptor/openai` + `minimax/`

**特性**:
- OpenAI 兼容 API
- 标准 endpoint: `/v1/text/chatcompletion_v2`
- 可能需要 group_id
...
**对应 One API**: `relay/adaptor/openai` + `doubao/`

**特性**:
- OpenAI 兼容 API
- 标准 endpoint: `/api/v3/chat/completions`
