...
"""
字节跳动豆包协议转换器
豆包使用 OpenAI 兼容 API，但 endpoint 不同
对应 One API 的 relay/adaptor/openai + doubao/main.go
"""
import os
...
class DoubaoConverter(ProtocolConverter):
    """字节跳动豆包协议转换器 - OpenAI 兼容"""
    
    def convert_request(self, request: OpenAIChatRequest) -> Dict[str, Any]:
        """
        豆包使用 OpenAI 兼容格式，直接返回
        """
        # 构建 OpenAI 格式请求
        request_dict = {
            "model": self.model,
    ...
    def convert_response(self, response_data: Dict[str, Any]) -> OpenAIChatResponse:
        ...
        if "error" in response_data:
            ...
            raise Exception(f"Doubao API error ({error_type}): {error_msg}")
    ...
    def get_request_url(self, base_url: str, endpoint: str) -> str:
        """构建请求 URL"""
        # 豆包的标准 endpoint 是 /api/v3/chat/completions
