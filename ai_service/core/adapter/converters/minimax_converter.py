...
"""
MINIMAX 协议转换器
MINIMAX 使用 OpenAI 兼容 API，但 endpoint 不同
对应 One API 的 relay/adaptor/openai + minimax/main.go
"""
import os
...
class MinimaxConverter(ProtocolConverter):
    """MINIMAX 协议转换器 - OpenAI 兼容"""
    
    def convert_request(self, request: OpenAIChatRequest) -> Dict[str, Any]:
        """
        MINIMAX 使用 OpenAI 兼容格式，直接返回
        """
        # 构建 OpenAI 格式请求
        request_dict = {
            "model": self.model,
    ...
    def convert_response(self, response_data: Dict[str, Any]) -> OpenAIChatResponse:
        ...
        if "error" in response_data:
            ...
            raise Exception(f"MINIMAX API error ({error_type}): {error_msg}")
    ...
    def get_request_headers(self) -> Dict[str, str]:
        ...
        # 直接使用配置文件中的 group_id，不再支持环境变量
        group_id = self.config.get("config", {}).get("group_id", "")
        if group_id == "not-needed":
            group_id = ""
        
        if group_id:
            headers["Authorization"] = f"Bearer {api_key}"
            # MINIMAX 的 group_id 可能需要在 URL 中传递或通过 header 传递
            # 根据实际情况调整
    ...
    def get_request_url(self, base_url: str, endpoint: str) -> str:
        """构建请求 URL"""
        # MINIMAX 的标准 endpoint 是 /v1/text/chatcompletion_v2
